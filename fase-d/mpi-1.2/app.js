...
function initStateArrays() {
  ...
  if (!State.eksplorasiRevealed || State.eksplorasiRevealed.length !== nEksp) {
    State.eksplorasiRevealed = DATA.eksplorasi.konteks.map(function () { return false; });
  }
  var nPola = DATA.polaOperasi.pola.length;
  if (!State.polaRevealed || State.polaRevealed.length !== nPola) {
    State.polaRevealed = DATA.polaOperasi.pola.map(function () { return false; });
  }
...
function buildOpNL(min, max, startVal, endVal) {
  ...
  var html = '<svg class="numberline-svg" viewBox="0 0 ' + W + " " + H + '" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">';

  /* Axis line */
  html += '<line class="nl-axis" x1="' + (padX - 10) + '" y1="' + axisY + '" x2="' + (W - padX + 10) + '" y2="' + axisY + '"/>';

  /* Arrow heads */
  html += '<polygon class="nl-arrow" points="' + (W - padX + 10) + "," + axisY + " " + (W - padX + 2) + "," + (axisY - 4) + " " + (W - padX + 2) + "," + (axisY + 4) + '"/>';
  html += '<polygon class="nl-arrow" points="' + (padX - 10) + "," + axisY + " " + (padX - 2) + "," + (axisY - 4) + " " + (padX - 2) + "," + (axisY + 4) + '"/>';
  ...
  for (var v = min; v <= max; v += step) {
    ...
    html += '<line class="nl-tick' + (isMajor ? " nl-tick--major" : "") + '" x1="' + x + '" y1="' + (axisY - tickH) + '" x2="' + x + '" y2="' + (axisY + tickH) + '"/>';
    if (isMajor) {
      var lCls = v === 0 ? "nl-label nl-label--zero" : "nl-label";
      html += '<text class="' + lCls + '" x="' + x + '" y="' + (axisY + tickH + 5) + '">' + v + "</text>";
    }
  }
  ...
  html += '<text class="nl-point-label--start" x="' + sx + '" y="' + (axisY - 16) + '">' + fmtNum(startVal) + "</text>";
  ...
  if (endVal !== null && endVal !== undefined) {
    ...
    /* Curved dashed arc */
    html += '<path class="nl-move-arc" d="M ' + sx + "," + startArcY + " Q " + midX + "," + arcY + " " + ex + "," + startArcY + '"/>';

    /* Arrowhead at end of arc */
    if (ex > sx) {
      /* Moving right: arrowhead points right-down */
      html += '<polygon class="nl-move-arc-head" points="' + ex + "," + startArcY + " " + (ex - 9) + "," + (startArcY - 6) + " " + (ex - 5) + "," + (startArcY + 6) + '"/>';
    } else {
    ...
    html += '<text class="nl-point-label--end" x="' + ex + '" y="' + (axisY - 16) + '">' + fmtNum(endVal) + "</text>";
  }
  ...
  var items = langkah
    .map(function (l, i) {
      return (
        ...
        '<span class="step-item__expr">' + esc(l) + "</span>" +
        "</div>"
      );
    })
    .join("");
...
function renderCurrentStage() {
  ...
  switch (State.currentStage) {
    ...
    case "polaOperasi":       renderPolaOperasi(container); break;
    case "latihanOperasi":    renderLatihanOperasi(container); break;
    case "masalahKontekstual": renderMasalah(container); break;
...
function renderEksplorasi(container) {
  ...
  var allRevealed =
    State.eksplorasiRevealed.filter(Boolean).length === ctxList.length;
  ...
  var tabsHTML = ctxList
    .map(function (c, i) {
      ...
      return (
        ...
        ' data-ctx-idx="' + i + '">' +
        '<span class="stage-nav__num">' + (isDone ? "✓" : i + 1) + "</span>" +
        esc(c.badge) +
        "</button>"
      );
    })
  ...
  var valueRowHTML =
    ...
    '<span class="op-val-box__num op-val-box__num--start">' + fmtNum(ctx.awal) + "</span>" +
    '<span style="font-size:0.75rem;color:var(--color-ink-muted);">' + esc(ctx.unit) + "</span>" +
    "</div>" +
    '<span class="op-arrow-icon">' + opSymbol + "</span>" +
    '<div class="op-val-box">' +
    '<span class="op-val-box__label">Perubahan</span>' +
    '<span class="op-val-box__num op-val-box__num--op">' + (ctx.op === "+" ? fmtNum(ctx.nilai) : "−" + ctx.nilai) + "</span>" +
    '<span style="font-size:0.75rem;color:var(--color-ink-muted);">' + esc(ctx.unit) + "</span>" +
    "</div>" +
    '<span class="op-arrow-icon">=</span>' +
    '<div class="op-val-box">' +
    '<span class="op-val-box__label">' + esc(ctx.kalimat) + "</span>" +
    (revealed
  ...
  container.innerHTML =
    ...
    '<span style="font-size:2rem;" aria-hidden="true">' + ctx.icon + "</span>" +
    "<div>" +
    ...
    valueRowHTML +
    "</div>" +
    '<div class="numberline-wrap">' + nlHTML + "</div>" +
...
function renderPolaOperasi(container) {
  ...
  var cardsHTML = pola
    .map(function (p, i) {
      ...
      return (
        '<div class="op-card' + (revealed ? " is-revealed" : "") + '" data-pola-idx="' + i + '">' +
        '<div class="op-card__header ' + hClass + '">' +
        '<span aria-hidden="true">' + p.icon + "</span> " +
        esc(p.judul) +
        "</div>" +
        '<div class="op-card__body">' +
        '<div class="op-card__examples">' +
        p.contoh.map(function (c) { return "<span>" + esc(c) + "</span>"; }).join("") +
        "</div>" +
        '<div class="op-card__formula">' + esc(p.formula) + "</div>" +
        (revealed
          ? '<div class="op-card__rule">' + p.aturan + "</div>" +
            '<div class="op-card__arah">' + esc(p.arah) + "</div>" +
            '<div class="op-card__ingatan">' + esc(p.ingatan) + "</div>"
          : '<div class="btn-group btn-group--center" style="margin-top:var(--space-3);">' +
            '<button type="button" class="btn btn--outline-primary btn--small" data-reveal-pola="' + i + '">Lihat Aturan →</button>' +
            "</div>") +
        "</div></div>"
      );
    })
  ...
  container.innerHTML =
    ...
    "</p>" +
    '<div class="op-cards-grid">' + cardsHTML + "</div>" +
    (allRevealed
      ? '<div class="btn-group btn-group--end">' +
...
function renderLatihanOperasi(container) {
  ...
  var allDoneOrRevealed =
    exArr.filter(function (e) { return e.correct || e.revealed; }).length === soal.length;
  ...
  if (ex.correct) {
    ...
    feedbackHTML = buildFeedbackBox("info", "👁", "<strong>Jawaban:</strong> " + s.answer + ". " + s.explanation);
  ...
  container.innerHTML =
    ...
    dotsHTML +
    '<div class="ex-row">' +
    '<div class="ex-expr">' + esc(s.ekspresi) + " = ?" + "</div>" +
    actionHTML +
    (feedbackHTML ? '<div style="margin-top:var(--space-3);">' + feedbackHTML + "</div>" : "") +
    "</div>" +
    "</div>" +
...
function renderMasalah(container) {
  ...
  var allDoneOrRevealed =
    exArr.filter(function (e) { return e.correct || e.revealed || e.chosen; }).length === soal.length;
  ...
  var problemHTML =
    ...
    '<span style="font-size:1.6rem;" aria-hidden="true">' + s.icon + "</span>" +
    '<div style="flex:1;min-width:0;">' +
    '<p style="font-size:0.95rem;margin:0;">' + s.story + "</p>" +
    "</div></div>" +
    '<p style="font-weight:700;font-size:0.95rem;margin:0;">' + s.question + "</p>" +
    "</div>";
  ...
  if (s.type === "input") {
    if (!ex.correct && !ex.revealed) {
      answerHTML =
        ...
        'aria-label="Jawaban" value="' + esc(ex.userInput) + '">' +
        (s.unit ? '<span style="font-size:0.88rem;color:var(--color-ink-muted);">' + esc(s.unit) + "</span>" : "") +
    ...
    if (ex.correct) {
      feedbackHTML =
        ...
        (s.langkah ? "<p style='font-weight:600;margin:var(--space-3) 0 var(--space-2);font-size:0.88rem;'>Langkah penyelesaian:</p>" + buildStepsHTML(s.langkah) : "");
    } else if (ex.revealed) {
      feedbackHTML =
        buildFeedbackBox("info", "👁", "<strong>Jawaban:</strong> " + s.answer + " " + (s.unit || "") + ". " + s.explanation) +
        (s.langkah ? "<p style='font-weight:600;margin:var(--space-3) 0 var(--space-2);font-size:0.88rem;'>Langkah penyelesaian:</p>" + buildStepsHTML(s.langkah) : "");
    } else if (ex.hintShown) {
      feedbackHTML = buildFeedbackBox("warning", "💡", "<strong>Petunjuk:</strong> " + s.hint);
    } else if (ex.attempts > 0) {
      feedbackHTML = buildFeedbackBox("error", "✗", "Jawabanmu <strong>" + esc(ex.userInput) + "</strong> belum tepat. Coba lagi.");
    }
    ...
    var choicesHTML = s.options
      .map(function (opt, oi) {
        ...
        return (
          '<button type="button" class="' + cls + '" data-opt="' + opt.id + '">' +
          '<span class="choice-letter">' + optLetters[oi] + "</span>" +
          esc(opt.label) +
          "</button>"
        );
      })
  ...
  if (isDone && idx < soal.length - 1) {
    navHTML = '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary" id="masalahNextBtn">Soal Berikutnya →</button></div>';
  } else if (allDoneOrRevealed) {
    navHTML =
      '<div class="btn-group btn-group--end" style="margin-top:var(--space-4);">' +
  ...
  if (inp && checkBtn) {
    ...
    checkBtn.addEventListener("click", function () {
      ...
      ex.userInput = inp.value;
      ex.attempts++;
      if (parsed.error) { showNotice("Masukkan bilangan bulat yang valid."); saveState(); return; }
...
function renderTantangan(container) {
  ...
  if (s.langkah) {
    stepsPreviewHTML =
      ...
      s.langkah.map(function (l) { return esc(l); }).join(" → ") +
      "</div>";
  }
  ...
  var problemHTML =
    ...
    '<span class="badge-chip">' + esc(s.badge) + "</span>" +
    '<div style="display:flex;gap:var(--space-3);align-items:flex-start;flex-wrap:wrap;margin-bottom:var(--space-3);">' +
    '<span style="font-size:1.8rem;" aria-hidden="true">' + s.icon + "</span>" +
    '<div style="flex:1;min-width:0;">' +
    '<p style="font-size:0.95rem;margin:0;">' + s.story + "</p>" +
    "</div></div>" +
    '<p style="font-weight:700;font-size:0.95rem;margin:0;">' + s.question + "</p>" +
  ...
  if (s.type === "input") {
    if (!ex.correct && !ex.revealed) {
      answerHTML =
        ...
        'aria-label="Jawaban tantangan" value="' + esc(ex.userInput) + '">' +
        (s.unit ? '<span style="font-size:0.82rem;color:var(--color-ink-muted);">' + esc(s.unit) + "</span>" : "") +
    ...
    if (ex.correct) {
      ...
      feedbackHTML =
        buildFeedbackBox("info", "👁", "<strong>Jawaban:</strong> " + s.answer + ". " + s.explanation) +
        "<p style='font-weight:600;margin:var(--space-3) 0 var(--space-2);font-size:0.88rem;'>Langkah penyelesaian:</p>" +
        buildStepsHTML(s.langkah);
    } else if (ex.hintShown) {
      feedbackHTML = buildFeedbackBox("warning", "💡", "<strong>Petunjuk:</strong> " + s.hint);
    ...
    if (done) {
      feedbackHTML =
        ex.correct
          ? buildFeedbackBox("success", "✓", "<strong>Luar biasa!</strong> " + s.explanation) +
            "<p style='font-weight:600;margin:var(--space-3) 0 var(--space-2);font-size:0.88rem;'>Langkah penyelesaian:</p>" + buildStepsHTML(s.langkah)
  ...
  var isDone = s.type === "input" ? (ex.correct || ex.revealed) : ex.chosen !== null;
  if (isDone && idx < soal.length - 1) {
    navHTML = '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary" id="tantanganNextBtn">Soal Berikutnya →</button></div>';
  } else if (allDoneOrRevealed) {
    navHTML =
      '<div class="btn-group btn-group--end" style="margin-top:var(--space-4);">' +
  ...
  var questionsHTML = refleksiSoal
    .map(function (r) {
      ...
      return (
        ...
        '<p style="font-weight:600;font-size:0.92rem;margin-bottom:var(--space-2);">' + r.question + "</p>" +
        '<textarea class="input-textarea" id="refleksi_' + r.id + '" placeholder="' + esc(r.placeholder) + '" rows="3">' +
        esc(saved) +
        "</textarea>" +
        "</div>"
      );
...
function renderSelesai(container) {
  var latihanDone = State.latihanExercises.filter(function (e) { return e.correct; }).length;
  var masalahDone = State.masalahExercises.filter(function (e) { return e.correct; }).length;
  var tantanganDone = State.tantanganExercises.filter(function (e) { return e.correct; }).length;
  ...
  container.innerHTML =
    ...
    '<div class="achievement-card__desc">' + eksplorasiDone + " dari " + DATA.eksplorasi.konteks.length + " konteks dijelajahi</div>" +
    "</div>" +
    '<div class="achievement-card">' +
    '<div class="achievement-card__icon">🧮</div>' +
    '<div class="achievement-card__title">Latihan Operasi</div>' +
    '<div class="achievement-card__desc">' + latihanDone + " dari " + DATA.latihanOperasi.soal.length + " soal dijawab benar</div>" +
    "</div>" +
    '<div class="achievement-card">' +
    '<div class="achievement-card__icon">📚</div>' +
    '<div class="achievement-card__title">Masalah Kontekstual</div>' +
    '<div class="achievement-card__desc">' + masalahDone + " dari " + DATA.masalahKontekstual.soal.length + " masalah diselesaikan</div>" +
    "</div>" +
    '<div class="achievement-card">' +
    '<div class="achievement-card__icon">🏆</div>' +
    '<div class="achievement-card__title">Tantangan</div>' +
    '<div class="achievement-card__desc">' + tantanganDone + " dari " + DATA.tantangan.soal.length + " tantangan diselesaikan</div>" +
    "</div>" +
    "</div></div>" +
...
(function init() {
  ...
  if (resetBtn) {
    resetBtn.addEventListener("click", function () {
      if (!window.confirm("Reset semua progress? Semua jawaban dan refleksi akan terhapus.")) return;
      clearState();
      buildStageNav();
      updateProgress();
      renderCurrentStage();
