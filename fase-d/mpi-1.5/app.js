...
function initProblemStates() {
    ...
    DATA.problems.forEach(function (p) {
        ...
        ps.push({
            ...
            phases: {
                identify: {
                    completed: false, attempts: 0, selected: [], correct: false
                },
                model: {
                    completed: false, attempts: 0, hintLevel: 0, selected: null, lastCheckedId: null, correct: false
                },
                order: {
                    completed: false, attempts: 0, hintLevel: 0, selected: null, lastCheckedId: null, correct: false,
                ...
                calc: {
                    completed: false, substeps: calcSubsteps
                },
                answer: {
                    completed: false, attempts: 0, hintLevel: 0, input: '', correct: false, revealed: false
                },
                verify: {
                    completed: false, attempts: 0, selected: null, lastCheckedId: null, correct: false
                }
            },
            totalHints: 0,
            totalAttempts: 0,
...
function saveState() {
    ...
    } catch (e) { /* simpan gagal, lanjutkan tanpa crash */ }
}
...
function loadState() {
    try {
        ...
        if (!Array.isArray(saved.problemStates) ||
            saved.problemStates.length !== DATA.problems.length) return false;
...
function navigateTo(stageId) {
    ...
    /* Maju: pastikan tahap sebelumnya sudah selesai */
...
function updateStageNav() {
    ...
    items.forEach(function (item) {
        ...
        /* Kunci tahap depan yang belum bisa diakses */
        if (idx > currentIdx && idx > 0 && !State.completedStages[STAGES[idx - 1]]) {
            item.disabled = true;
        }
    });
}
...
function buildFracBlock(num, den, whole, size) {
    ...
    return '<span class="' + cls + '" role="img" aria-label="' + ariaLabel + '">' +
        wholeHTML +
...
function buildHints(hints, level) {
    ...
    for (var i = 0; i < level && i < hints.length; i++) {
        items += '<div class="hint-item"><span class="hint-item__num">Petunjuk ' + (i + 1) + '</span><span>' + esc(hints[i]) + '</span></div>';
    }
...
function buildProblemDots(currentIdx, problemStates) {
    ...
    problemStates.forEach(function (ps, i) {
        ...
        html += '<span class="' + cls + '" aria-label="Soal ' + (i + 1) + (ps.status === 'complete' ? ' (selesai)' : '') + '">' + (i + 1) + '</span>';
    });
    return html;
}
...
function renderCurrentStage() {
    ...
    switch (State.currentStage) {
        ...
        default:
            container.innerHTML = '<p style="text-align:center;padding:var(--space-7);color:var(--color-ink-muted);">Tahap tidak ditemukan.</p>';
    }
}
...
function renderOrientasi(container) {
    container.innerHTML =
        ...
        '<p style="font-size:1rem;margin-bottom:0;">' + esc(DATA.meta.goal) + '</p>' +
        '</div>' +
...
function renderContoh(container) {
    ...
    var html =
        ...
        '<div class="problem-card__kicker"><span>' + esc(ex.title) + '</span></div>' +
        '<p class="problem-card__context">' + ex.context + '</p>' +
        '<p class="problem-card__question">' + esc(ex.question) + '</p>' +
        '</div>' +

        '<div style="display:flex;align-items:center;gap:var(--space-3);margin-bottom:var(--space-4);">' +
        '<span style="font-size:0.82rem;color:var(--color-ink-muted);white-space:nowrap;">Langkah ' + (stepIdx + 1) + ' dari ' + totalSteps + '</span>' +
...
function buildContohStep(step) {
    ...
    if (step.type === 'read') {
        ...
        html += '<button type="button" class="btn btn--primary" id="contohNextBtn">' + esc(step.cta) + ' →</button>';
        html += '</div>';
    }
    ...
    if (step.type === 'mc') {
        ...
        step.options.forEach(function (opt) {
            ...
            html += '<button type="button" class="' + cls + '" data-optid="' + esc(opt.id) + '"' +
                (isLocked ? ' disabled' : '') + '>' +
                '<span class="option-card__marker">' + esc(opt.id.toUpperCase()) + '</span>' +
                '<span class="option-card__body"><span class="option-card__expr">' + esc(opt.label) + '</span></span>' +
                '</button>';
        });
        html += '</div>';

        if (stepResult) {
            var selOpt = step.options.find(function (o) { return o.id === stepResult.selectedId; });
            if (selOpt) {
        ...
        if (!stepResult) {
            ...
            if (hintText) html += '<button type="button" class="btn btn--ghost btn--small" id="contohHintBtn">Petunjuk</button>';
            html += '</div>';
        } else if (stepResult.correct) {
            html += '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary" id="contohNextBtn">Lanjut →</button></div>';
        }
    }
    ...
    if (step.type === 'input') {
        ...
        if (isDone) {
            ...
            var errorMsg = (stepResult2 && !stepResult2.correct && stepResult2.errorMsg) ? stepResult2.errorMsg : '';
            html += '<div class="calc-input-row">';
            html += '<label for="contohInput">' + esc(step.inputLabel) + '</label>';
            html += '<input type="text" id="contohInput" class="input-text' + (errorMsg ? ' has-error' : '') + '"' +
                ' placeholder="' + esc(step.inputPlaceholder) + '"' +
            ...
            if (hintLevel < step.hints.length) {
                html += '<button type="button" class="btn btn--ghost btn--small" id="contohHintBtn">Petunjuk</button>';
            }
    ...
    if (step.type === 'summary') {
        html += '<p style="color:var(--color-ink-muted);font-size:0.9rem;margin-bottom:var(--space-4);">Berikut adalah rangkuman penyelesaian lengkap dari contoh di atas.</p>';
        html += '<ol class="solution-steps">';
        step.steps.forEach(function (s) {
            html += '<li class="solution-step"><span class="solution-step__icon">→</span><span class="solution-step__text">' + esc(s) + '</span></li>';
        });
        html += '</ol>';
        if (step.note) {
            html += '<div class="contoh-explanation"><strong>Catatan:</strong> ' + esc(step.note) + '</div>';
        }
        html += '<div class="btn-group btn-group--end">';
        html += '<button type="button" class="btn btn--primary btn--large" id="contohDoneBtn">Mulai Latihan →</button>';
        html += '</div>';
    }
    ...
    if (step.type === 'mc') {
        document.querySelectorAll('.option-card[data-optid]').forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                saveState();
                renderContoh(document.getElementById('stageContainer'));
                window.scrollTo({ top: document.getElementById('contohStepContent').offsetTop - 120, behavior: 'smooth' });
            });
        });
        ...
        if (hintBtn) {
            hintBtn.addEventListener('click', function () {
                ...
                if (hintArea) hintArea.innerHTML = '<div class="hint-item"><span class="hint-item__num">Petunjuk</span><span>' + esc(step.hint) + '</span></div>';
    ...
    if (step.type === 'input') {
        ...
        if (checkBtn) {
            checkBtn.addEventListener('click', function () {
                ...
                if (!result.correct && existing.attempts >= MAX_ATTEMPTS_BEFORE_HINT && existing.hintLevel < step.hints.length) {
...
function checkContohInput(step, raw) {
    ...
    if (parsed.error === 'empty') return { correct: false, errorMsg: 'Masukkan jawaban terlebih dahulu.' };
    if (parsed.error === 'invalid') return { correct: false, errorMsg: 'Format tidak valid. Gunakan angka (koma atau titik untuk desimal). Contoh: 0,75' };

    if (step.answerType === 'numerator') {
        var intVal = Math.round(parsed.value);
        if (Math.abs(parsed.value - intVal) > 0.001) return { correct: false, errorMsg: 'Masukkan bilangan bulat untuk pembilang.' };
...
function renderLatihan(container) {
    ...
    if (pIdx < 0 || pIdx >= DATA.problems.length) { pIdx = 0; State.currentProblemIdx = 0; }

    var problem = DATA.problems[pIdx];
    var pstate = State.problemStates[pIdx];

    if (!pstate) {
        container.innerHTML = '<p style="color:var(--color-error-strong);padding:var(--space-5);">Terjadi kesalahan. <button type="button" class="btn btn--ghost btn--small" onclick="clearState();location.reload();">Reset</button></p>';
        return;
    }
    ...
    html += '<span style="font-size:0.82rem;color:var(--color-ink-muted);margin-left:var(--space-2);">Soal ' + (pIdx + 1) + ' dari ' + DATA.problems.length + '</span>';
    html += '</div>';
    /* Difficulty */
    var diff = problem.difficulty || 1;
    var diffLabel = diff === 1 ? 'Mudah' : diff === 2 ? 'Sedang' : 'Sulit';
    html += '<span style="font-size:0.82rem;color:var(--color-ink-muted);">Tingkat: <strong>' + esc(diffLabel) + '</strong></span>';
    html += '</div>';
...
function buildPhaseContent(problem, pstate, phase) {
    switch (phase) {
        case 'identify': return buildPhaseIdentify(problem, pstate);
        case 'model': return buildPhaseModel(problem, pstate);
        case 'order': return buildPhaseOrder(problem, pstate);
        case 'calc': return buildPhaseCalc(problem, pstate);
        case 'answer': return buildPhaseAnswer(problem, pstate);
        case 'verify': return buildPhaseVerify(problem, pstate);
        default:
...
function buildPhaseIdentify(problem, pstate) {
    ...
    identify.items.forEach(function (item) {
        ...
        else if (isLocked && !isChecked && item.relevant) extraCls = ' is-wrong-revealed'; /* yang seharusnya dipilih tapi tidak */
        else if (isChecked) extraCls = ' is-checked';

        var cls = 'identify-item' + extraCls + (isLocked ? ' is-locked' : '');
        html += '<li><button type="button" class="' + cls + '" data-itemid="' + esc(item.id) + '" role="checkbox" aria-checked="' + isChecked + '">';
    ...
    if (ph.completed) {
        ...
        html += '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary" id="phaseNextBtn">Lanjut: Tentukan Operasi →</button></div>';
    } else {
        if (ph.attempts > 0 && !ph.correct) {
            html += buildFeedbackBox('error', '✗', esc(identify.feedbackWrong));
        }
        var hintLevel = (ph.attempts >= MAX_ATTEMPTS_BEFORE_HINT) ? Math.min(identify.hints.length, ph.attempts - MAX_ATTEMPTS_BEFORE_HINT + 1) : 0;
        html += buildHints(identify.hints, hintLevel);
        html += '<div class="btn-group">';
        if (hintLevel < identify.hints.length && ph.attempts >= 1) {
            html += '<button type="button" class="btn btn--ghost btn--small" id="hintBtn">Petunjuk</button>';
        }
        html += '<button type="button" class="btn btn--primary" id="phaseCheckBtn"' + (ph.selected.length === 0 ? ' disabled' : '') + '>Periksa Pilihan</button>';
        html += '</div>';
    }
    html += '</div>';
    return html;
...
function buildPhaseModel(problem, pstate) {
    ...
    model.options.forEach(function (opt) {
        ...
        html += '<span class="option-card__body"><span class="option-card__expr">' + opt.expr + '</span></span>';
        html += '</button>';
    });
    html += '</div>';
    ...
    if (ph.attempts > 0 && ph.lastCheckedId) {
        var selOpt = model.options.find(function (o) { return o.id === ph.lastCheckedId; });
        if (selOpt) {
            html += buildFeedbackBox(
                selOpt.correct ? 'success' : 'error',
                selOpt.correct ? '✓' : '✗',
    ...
    if (ph.completed) {
        html += '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary" id="phaseNextBtn">Lanjut →</button></div>';
    } else {
        html += '<div class="btn-group">';
        if (ph.hintLevel < model.hints.length && ph.attempts >= 1) {
            html += '<button type="button" class="btn btn--ghost btn--small" id="hintBtn">Petunjuk</button>';
        }
        html += '<button type="button" class="btn btn--primary" id="phaseCheckBtn" ' + (!ph.selected ? 'disabled' : '') + '>Periksa</button>';
        html += '</div>';
    }
    html += '</div>';
    return html;
...
function buildPhaseOrder(problem, pstate) {
    ...
    order.options.forEach(function (opt) {
        ...
        html += '<button type="button" class="' + cls + '" data-optid="' + esc(opt.id) + '">';
        html += '<span class="option-card__marker">' + esc(opt.id.toUpperCase()) + '</span>';
        html += '<span class="option-card__body"><span style="font-size:0.95rem;">' + esc(opt.label) + '</span></span>';
        html += '</button>';
    });
    html += '</div>';

    if (ph.attempts > 0 && ph.lastCheckedId) {
        var selOpt = order.options.find(function (o) { return o.id === ph.lastCheckedId; });
        if (selOpt) {
            html += buildFeedbackBox(
                selOpt.correct ? 'success' : 'error',
                selOpt.correct ? '✓' : '✗',
    ...
    if (ph.completed) {
        html += '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary" id="phaseNextBtn">Lanjut: Hitung Bertahap →</button></div>';
    } else {
        html += '<div class="btn-group">';
...
function buildPhaseCalc(problem, pstate) {
    ...
    html += '<p style="color:var(--color-ink-muted);font-size:0.9rem;">' + esc(calc.instruction) + '</p>';
    ...
    calc.steps.forEach(function (step, i) {
        ...
        if (isDone) {
            ...
            html += '<input type="text" id="calcInput' + i + '"' +
                ' class="input-text' + (errorMsg ? ' has-error' : '') + '"' +
                ' placeholder="' + esc(step.inputPlaceholder) + '"' +
                ' value="' + esc(ss.input || '') + '"' +
                ' autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false"' +
                ' data-substep="' + i + '">';
            ...
            if (ss.hintLevel < step.hints.length && ss.attempts >= 1) {
                html += '<button type="button" class="btn btn--ghost btn--small" id="calcHintBtn' + i + '" data-substep="' + i + '">Petunjuk</button>';
            }
            html += '<button type="button" class="btn btn--primary" id="calcCheckBtn' + i + '" data-substep="' + i + '">Periksa</button>';
            html += '</div>';
        } else {
            /* Sub-langkah belum bisa diakses */
            html += '<p style="color:var(--color-ink-muted);font-size:0.88rem;font-style:italic;">Selesaikan langkah sebelumnya terlebih dahulu.</p>';
        }
    ...
    if (ph.completed) {
        html += '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary" id="phaseNextBtn">Lanjut: Jawaban Akhir →</button></div>';
    }
    html += '</div>';
    return html;
}
...
function buildPhaseAnswer(problem, pstate) {
    ...
    if (isDone) {
        html += '<div class="calc-step__result-display" style="font-size:1.1rem;padding:var(--space-3) var(--space-4);">✓ ' + esc(answer.correctDisplay) + (answer.unit ? ' ' + answer.unit : '') + '</div>';
        html += buildFeedbackBox('success', '✓', esc(answer.feedbackCorrect));
        html += '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary" id="phaseNextBtn">Lanjut: Periksa Kewajaran →</button></div>';
    } else {
        var errorMsg = ph.errorMsg || '';
        html += '<div class="field-group">';
        html += '<label for="answerInput">' + esc(answer.inputLabel) + (answer.unit ? ' (' + answer.unit + ')' : '') + '</label>';
        html += '<div class="calc-input-row" style="margin-top:var(--space-2);">';
        html += '<input type="text" id="answerInput" class="input-text' + (errorMsg ? ' has-error' : '') + '"' +
            ' placeholder="' + esc(answer.inputPlaceholder) + '"' +
            ' value="' + esc(ph.input || '') + '"' +
            ' autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false"' +
            ' style="max-width:220px;">';
        if (answer.unit) html += '<span style="font-size:0.95rem;color:var(--color-ink-muted);">' + esc(answer.unit) + '</span>';
        html += '</div>';
        html += '</div>';
        ...
        html += '<button type="button" class="btn btn--primary" id="phaseCheckBtn">Periksa Jawaban</button>';
        html += '</div>';
    }
    html += '</div>';
    return html;
...
function buildPhaseVerify(problem, pstate) {
    ...
    verify.options.forEach(function (opt) {
        ...
        html += '<span class="option-card__body"><span style="font-size:0.9rem;">' + esc(opt.text) + '</span></span>';
        html += '</button>';
    });
    html += '</div>';

    if (ph.attempts > 0 && ph.lastCheckedId) {
        var selOpt = verify.options.find(function (o) { return o.id === ph.lastCheckedId; });
        if (selOpt) {
            html += buildFeedbackBox(
                selOpt.correct ? 'success' : 'error',
                selOpt.correct ? '✓' : '✗',
    ...
    if (ph.completed) {
        html += '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary btn--large" id="phaseNextBtn">Selesai — Lihat Rangkuman →</button></div>';
    } else {
        html += '<div class="btn-group btn-group--end">';
        html += '<button type="button" class="btn btn--primary" id="phaseCheckBtn" ' + (ph.selected ? '' : 'disabled') + '>Periksa</button>';
        html += '</div>';
...
function renderProblemReview(problem, pstate, container) {
    ...
    var allDone = State.problemStates.every(function (ps) { return ps.status === 'complete'; });

    var html =
        '<section aria-label="Rangkuman Soal">' +
        '<div class="stage-head">' +
        '<span class="stage-head__kicker">TAHAP 3 — LATIHAN</span>' +
        '<p class="stage-head__goal">Rangkuman penyelesaian soal ' + (pIdx + 1) + '.</p>' +
        '</div>';
    ...
    /* Problem card */
    html += '<div class="problem-card">';
    html += '<div class="problem-card__kicker"><span>Soal ' + (pIdx + 1) + ': ' + esc(problem.title) + ' — Selesai</span></div>';
    html += '<p class="problem-card__context">' + problem.context + '</p>';
    html += '<p class="problem-card__question">' + esc(problem.question) + '</p>';
    html += '</div>';
    ...
    html += '<div><span style="font-size:1.5rem;font-weight:700;color:var(--color-success-strong);">✓</span> <span style="font-size:0.9rem;color:var(--color-success-strong);">Soal selesai</span></div>';
    html += '<div><span style="font-size:0.9rem;color:var(--color-ink-muted);">Total percobaan: </span><strong>' + totalAttempts + '</strong></div>';
    html += '<div><span style="font-size:0.9rem;color:var(--color-ink-muted);">Petunjuk digunakan: </span><strong>' + totalHints + '</strong></div>';
    html += '</div></div>';
    ...
    problem.solution.steps.forEach(function (step) {
        html += '<li class="solution-step"><span class="solution-step__icon">→</span><span class="solution-step__text">' + esc(step) + '</span></li>';
    });
    html += '</ol></div>';
    ...
    html += '<div class="btn-group">';
    if (pIdx > 0) {
        html += '<button type="button" class="btn btn--ghost" id="prevProblemBtn">← Soal Sebelumnya</button>';
    }
    html += '<button type="button" class="btn btn--outline-primary" id="retryProblemBtn">Ulangi Soal Ini</button>';
    html += '</div>';
    html += '<div class="btn-group btn-group--end">';
    if (!isLastProblem) {
        html += '<button type="button" class="btn btn--primary btn--large" id="nextProblemBtn">Soal Berikutnya →</button>';
    } else {
        html += '<button type="button" class="btn btn--primary btn--large" id="allDoneBtn">Lihat Hasil Akhir →</button>';
    }
    html += '</div></div>';
...
function resetProblemState(pIdx) {
    ...
    State.problemStates[pIdx] = {
        ...
        phases: {
            ...
            model: { completed: false, attempts: 0, hintLevel: 0, selected: null, lastCheckedId: null, correct: false },
            order: {
                completed: false, attempts: 0, hintLevel: 0, selected: null, lastCheckedId: null, correct: false,
                skipped: (problem.order === null)
            },
            calc: { completed: false, substeps: calcSubsteps },
            answer: { completed: false, attempts: 0, hintLevel: 0, input: '', correct: false, revealed: false },
            verify: { completed: false, attempts: 0, selected: null, lastCheckedId: null, correct: false }
        },
        totalHints: 0,
        totalAttempts: 0,
        correctFirstTry: 0
...
function checkIdentify(problem, pstate) {
    ...
    var relevant = identify.items.filter(function (i) { return i.relevant; }).map(function (i) { return i.id; });
    var notRelevant = identify.items.filter(function (i) { return !i.relevant; }).map(function (i) { return i.id; });

    var allRelevantSelected = relevant.every(function (id) { return ph.selected.indexOf(id) !== -1; });
    var noIrrelevantSelected = notRelevant.every(function (id) { return ph.selected.indexOf(id) === -1; });
...
function checkMCPhase(problem, pstate, phase) {
    ...
    var source = phase === 'model' ? problem.model.options :
        phase === 'order' ? problem.order.options :
            problem.verify.options;

    var selOpt = source.find(function (o) { return o.id === ph.selected; });
...
function checkCalcSubstep(problem, pstate, substepIdx) {
    ...
    if (ss.correct) {
        ...
        var allDone = ph.substeps.every(function (s) { return s.correct; });
        if (allDone) {
            ph.completed = true;
        }
    } else {
    ...
    if (checkBtn) {
        checkBtn.addEventListener('click', function () {
            ...
            if (parsed.error === 'empty') {
                ...
                ph.errorMsg = '';
                ph.correct = approxEqual(parsed.value, problem.answer.answer, problem.answer.tolerance || 0.01);
            }
...
function addHint(pstate, phase) {
    ...
    var problem = DATA.problems[pIdx];
    switch (phase) {
        case 'identify': sourceHints = problem.identify.hints; break;
        case 'model': sourceHints = problem.model.hints; break;
        case 'order': sourceHints = problem.order ? problem.order.hints : []; break;
        case 'answer': sourceHints = problem.answer.hints; break;
        default: return;
    }
...
function renderHasil(container) {
    ...
    var completed = State.problemStates.filter(function (ps) { return ps.status === 'complete'; }).length;
    var totalHints = State.problemStates.reduce(function (acc, ps) { return acc + ps.totalHints; }, 0);
    var totalAttempts = State.problemStates.reduce(function (acc, ps) { return acc + ps.totalAttempts; }, 0);
    var correctFirstTry = State.problemStates.reduce(function (acc, ps) { return acc + ps.correctFirstTry; }, 0);
    ...
    var html =
        ...
        '<p>' + esc(completed + ' dari ' + totalProblems + ' soal berhasil diselesaikan.') + '</p>' +
        '</div>' +
    ...
    State.problemStates.forEach(function (ps, i) {
        ...
        html += '<span style="color:' + (ps.status === 'complete' ? 'var(--color-success-strong)' : 'var(--color-ink-muted)') + ';">' + status + '</span>';
        html += ' <span style="color:var(--color-ink-muted);font-size:0.85rem;">| Percobaan: ' + ps.totalAttempts + ', Petunjuk: ' + ps.totalHints + '</span>';
        html += '</li>';
    });
    html += '</ol></div>';
    ...
    html += '<strong>Catatan:</strong> Hasil di atas adalah ringkasan latihan mandiri dalam media ini. ';
    html += 'Ini bukan nilai akhir sekolah. Guru kamu tetap akan menilai kemampuanmu melalui berbagai bentuk asesmen lainnya.';
    html += '</p></div>';

    html += '<div class="btn-group btn-group--spread">';
    html += '<button type="button" class="btn btn--ghost" id="backToLatihanBtn">← Kembali ke Latihan</button>';
    html += '<button type="button" class="btn btn--primary btn--large" id="goRefleksiBtn">Lanjut ke Refleksi →</button>';
    html += '</div>';
...
function buildStatCard(value, label) {
    return '<div class="stat-card"><div class="stat-card__num">' + esc(String(value)) + '</div><div class="stat-card__label">' + esc(label) + '</div></div>';
}
...
function renderRefleksi(container) {
    ...
    refleksi.questions.forEach(function (q) {
        ...
        if (q.type === 'mc') {
            q.options.forEach(function (opt, i) {
                ...
                html += '<label style="display:flex;align-items:center;gap:var(--space-2);margin-bottom:var(--space-2);cursor:pointer;font-size:0.92rem;">';
                html += '<input type="radio" name="refleksi_' + esc(q.id) + '" value="' + i + '"' + (isChecked ? ' checked' : '') + (isSaved ? ' disabled' : '') + '>';
                html += esc(opt);
                html += '</label>';
            });
        }
        ...
        if (q.type === 'text') {
            html += '<textarea class="input-textarea" name="refleksi_' + esc(q.id) + '" placeholder="' + esc(q.placeholder) + '"' +
                (isSaved ? ' disabled' : '') + '>' +
                esc(saved[q.id] || '') + '</textarea>';
        }
    ...
    if (!isSaved) {
        html += '<div class="btn-group btn-group--end">';
        html += '<button type="button" class="btn btn--primary btn--large" id="saveRefleksiBtn">Simpan Refleksi & Selesai</button>';
        html += '</div>';
    } else {
        html += '<div class="panel panel--success panel--compact">';
        html += '<p style="margin:0;">Refleksi tersimpan. Terima kasih telah menyelesaikan sesi latihan ini!</p>';
        html += '</div>';
        html += '<div class="btn-group btn-group--center">';
        html += '<button type="button" class="btn btn--outline-primary" id="backHasilBtn">← Kembali ke Hasil</button>';
        html += '</div>';
    }
    ...
    if (backBtn) {
        backBtn.addEventListener('click', function () { navigateTo('hasil'); });
    }
}
...
function buildStageNav() {
    ...
    STAGES.forEach(function (sid, i) {
        html += '<li>';
        html += '<button type="button" class="stage-nav__item" data-stage="' + esc(sid) + '">';
        html += '<span class="stage-nav__num">' + (i + 1) + '</span>';
        html += esc(STAGE_LABELS[i]);
        html += '</button>';
        html += '</li>';
    });
    list.innerHTML = html;
