...
var State = {
  ...
  presentasiNarasi: '',
  presentasiAnswers: [{ input: '', done: false }, { input: '', done: false }],
  presentasiRefleksi: '',
...
function initExerciseArrays() {
  ...
  if (!State.presentasiAnswers || State.presentasiAnswers.length !== 2) {
    State.presentasiAnswers = [{ input: '', done: false }, { input: '', done: false }];
  }
...
function renderCurrentStage() {
  ...
  switch (State.currentStage) {
    ...
    case 'eksplorasiOperasi': renderEksplorasiOperasi(container); break;
    case 'menyusunRincian': renderMenyusunRincian(container); break;
    case 'analisisAnggaran': renderAnalisisAnggaran(container); break;
    case 'finalisasiRancangan': renderFinalisasiRancangan(container); break;
    case 'presentasiRancangan': renderPresentasiRancangan(container); break;
...
function buildMasalahCard(icon, label, desc) {
  return (
    ...
    '<div style="font-weight:700;font-size:0.85rem;margin-bottom:4px;">' + esc(label) + '</div>' +
    '<div style="font-size:0.78rem;color:var(--color-ink-muted);">' + esc(desc) + '</div>' +
    '</div>'
  );
}
...
function renderEksplorasiOperasi(container) {
  ...
  var tabsHTML = ctxList.map(function (c, i) {
    ...
    return (
      '<button type="button" class="stage-nav__item' + done + '"' + active + ' data-ctx="' + i + '">' +
  ...
  var itemsHTML = ctx.items.map(function (item) {
    var valHTML;
    if (item.harga_label) {
      valHTML = '<span class="receipt-item__val receipt-item__val--label">' + esc(item.harga_label) + '</span>';
    } else {
      valHTML = '<span class="receipt-item__val">' + formatRupiah(item.harga) + '</span>';
    }
    return (
      '<div class="receipt-item">' +
      '<span class="receipt-item__name">' + esc(item.nama) + '</span>' +
      valHTML +
      '</div>'
    );
  }).join('');
  ...
  for (var i = 0; i < ctx.steps.length; i++) {
    ...
    if (i < currentStep || (isDone && i < ctx.steps.length)) {
      stepsHTML +=
        ...
        '<p style="margin-bottom:var(--space-2);">' + s.question + '</p>' +
        buildFeedbackBox('success', '&#10003;', s.explanation) +
        '</div></div>';
    } else if (i === currentStep && !isDone) {
      stepsHTML +=
        '<div class="ops-step ops-step--active" id="activeStep">' +
        '<div class="ops-step__num">' + (i + 1) + '</div>' +
        '<div class="ops-step__content">' +
        '<p style="margin-bottom:var(--space-3);">' + s.question + '</p>' +
        '<div class="input-row">' +
  ...
  container.innerHTML =
    ...
    esc(DATA.eksplorasiOperasi.instruction) + '</p>' +

    '<div class="ctx-tabs" id="ctxTabs">' + tabsHTML + '</div>' +
    ...
    esc(ctx.operasi_label) + '</span>' +
    '<p style="margin:var(--space-2) 0 0;font-size:0.92rem;">' + ctx.story + '</p>' +
    '</div></div>' +

    '<div class="receipt-card">' +
    '<div class="receipt-card__header">' +
    ctx.icon + ' ' + esc(ctx.badge) + ' — Data Anggaran' +
    '</div>' +
    itemsHTML +
    '</div></div>' +
...
function handleEksplorasiCheck(container, ctxIdx, stepIdx) {
  ...
  if (parsed.error === 'invalid') {
    fb.innerHTML = buildFeedbackBox('error', '&#9888;', 'Ketik angka saja (contoh: <strong>295000</strong> atau <strong>295.000</strong>).');
    return;
  }
...
function renderMenyusunRincian(container) {
  ...
  var allDone = State.menyusunExercises.every(function (e) { return e.correct || e.revealed; });
  ...
  if (ex.correct) {
    ...
    feedbackHTML = buildFeedbackBox('info', '&#128218;',
  ...
  container.innerHTML =
    ...
    '<div class="panel">' +
    '<p style="font-size:0.88rem;color:var(--color-ink-muted);margin-bottom:var(--space-4);">' +
    esc(DATA.menyusunRincian.instruction) + '</p>' +
    dotsHTML +

    '<div class="panel panel--hero" style="margin-bottom:var(--space-4);">' +
    '<div style="display:flex;align-items:center;gap:var(--space-3);margin-bottom:var(--space-3);flex-wrap:wrap;">' +
    '<span style="font-size:1.6rem;">' + soal.icon + '</span>' +
    '<div>' +
    '<span class="badge-chip">' + esc(soal.kategori) + '</span> ' +
    operasiHTML +
    '</div></div>' +
    ...
    soal.konteks + '</p>' +
    '<p style="font-weight:600;">' + soal.question + '</p>' +
    ...
    (inputDisabled ? 'disabled' : '') + '>Periksa</button>' +
    '</div>' +
    ...
    (!ex.correct && !ex.revealed && ex.hintLevel < soal.hints.length
      ? '<button type="button" class="btn btn--ghost btn--small" id="menyusunHintBtn">&#128161; Petunjuk (' + (ex.hintLevel + 1) + '/' + soal.hints.length + ')</button>'
      : ''
    ) +
  ...
  if (checkBtn) {
    checkBtn.addEventListener('click', function () { handleMenyusunCheck(container, idx); });
  ...
  attachNavRowListeners(container, idx, soalList.length, 'menyusunIdx', renderMenyusunRincian, 'menyusunRincian', 'analisisAnggaran');
}
...
function handleMenyusunCheck(container, idx) {
  ...
  if (parsed.error === 'empty') {
    if (fb) fb.innerHTML = buildFeedbackBox('error', '&#9888;', 'Masukkan jawaban terlebih dahulu.');
    return;
  }
  if (parsed.error === 'invalid') {
    if (fb) fb.innerHTML = buildFeedbackBox('error', '&#9888;', 'Ketik angka saja. Contoh: <strong>140000</strong> atau <strong>140.000</strong>.');
    return;
  }
...
function renderAnalisisAnggaran(container) {
  ...
  var dotsHTML = buildProgressDots(soalList.length, idx, statuses);
  var allDone = State.analisisExercises.every(function (e) { return e.correct || e.revealed || e.checked; });
  ...
  container.innerHTML =
    ...
    '<div class="panel">' +
    '<p style="font-size:0.88rem;color:var(--color-ink-muted);margin-bottom:var(--space-4);">' +
    esc(DATA.analisisAnggaran.instruction) + '</p>' +
    dotsHTML +

    '<div class="panel panel--compact" style="margin-bottom:var(--space-4);">' +
    '<p style="font-weight:600;font-size:0.95rem;margin-bottom:var(--space-4);">' + soal.question + '</p>' +
    exerciseBody +
    '</div>' +
  ...
  attachNavRowListeners(container, idx, soalList.length, 'analisisIdx', renderAnalisisAnggaran, 'analisisAnggaran', 'finalisasiRancangan');
}
...
function buildAnalisisInput(soal, ex) {
  ...
  if (!ex.correct && !ex.revealed) {
    if (ex.hintLevel < hints.length) {
      hintHTML += '<button type="button" class="btn btn--ghost btn--small" id="analisisHintBtn">&#128161; Petunjuk (' + (ex.hintLevel + 1) + '/' + hints.length + ')</button>';
    }
    if (ex.attempts >= 2) {
      hintHTML += '<button type="button" class="btn btn--ghost btn--small" id="analisisRevealBtn">&#128218; Lihat Jawaban</button>';
    }
  }
  return (
    '<div class="input-row" style="margin-bottom:var(--space-3);">' +
    ...
    (hintHTML ? '<div class="btn-group" style="margin-top:var(--space-3);">' + hintHTML + '</div>' : '')
  );
}
  ...
  if (ex.checked) {
    if (ex.correct) {
      ...
      feedbackHTML = buildFeedbackBox('error', '&#10007;',
        'Belum tepat. ' + soal.hint + '<br><button type="button" class="btn btn--ghost btn--small" id="analisisTryAgainBtn" style="margin-top:var(--space-2);">Coba Lagi</button>'
      );
    }
  }
  var optionsHTML = soal.options.map(function (opt) {
    var selected = ex.chosen === opt.id;
    var cls = 'choice-btn' + (selected ? ' is-selected' : '');
    var disabled = ex.checked && ex.correct ? 'disabled' : '';
    return (
      '<button type="button" class="' + cls + '" data-opt="' + opt.id + '" ' + disabled + '>' +
      esc(opt.label) +
      '</button>'
    );
  }).join('');
  return (
    '<div class="choice-group" style="margin-bottom:var(--space-3);">' + optionsHTML + '</div>' +
...
function attachAnalisisListeners(container, idx, soal) {
  ...
  if (checkBtn) {
    checkBtn.addEventListener('click', function () { handleAnalisisCheck(container, idx); });
    var inp = document.getElementById('analisisInput');
    if (inp) {
      inp.addEventListener('keydown', function (e) { if (e.key === 'Enter') handleAnalisisCheck(container, idx); });
...
function handleAnalisisCheck(container, idx) {
  ...
  if (parsed.error === 'invalid') {
    if (fb) fb.innerHTML = buildFeedbackBox('error', '&#9888;', 'Ketik angka saja. Contoh: <strong>20000</strong>.');
    return;
  }
  ex.attempts++;
  if (parsed.value === soal.answer) {
...
function calcFinalisasiTotals() {
  ...
  State.finalisasiItems.forEach(function (item) {
    ...
    var t = (q.error === null && h.error === null && q.value > 0 && h.value > 0)
      ? q.value * h.value : 0;
    item.total = t;
    grand += t;
  });
...
function renderFinalisasiRancangan(container) {
  ...
  var sisaClass = overBudget ? 'budget-bar-labels__remaining--over' : 'budget-bar-labels__remaining';
  var sisaText = overBudget
    ? '&#9888; Melebihi anggaran ' + formatRupiah(grand - batas)
    : 'Sisa ' + formatRupiah(sisa);

  var rowsHTML = kata.kategori.map(function (kat, i) {
    ...
    return (
      '<tr data-row="' + i + '">' +
      '<td class="cat-cell">' + kat.icon + ' ' + esc(kat.nama) +
      '<span class="cat-cell__hint">' + esc(kat.hint_contoh) + '</span></td>' +
      '<td class="input-cell">' +
      '<input type="text" inputmode="numeric" class="budget-input qty-input" ' +
      'data-row="' + i + '" value="' + esc(item.qty) + '" ' +
      (State.finalisasiDone ? 'disabled ' : '') +
      'placeholder="Jml" aria-label="Jumlah ' + esc(kat.nama) + '" />' +
      '</td>' +
      '<td class="input-cell">' +
      '<input type="text" inputmode="numeric" class="budget-input harga-input" ' +
      'data-row="' + i + '" value="' + esc(item.harga) + '" ' +
      (State.finalisasiDone ? 'disabled ' : '') +
      'placeholder="Harga" aria-label="Harga satuan ' + esc(kat.nama) + '" />' +
      '</td>' +
      '<td class="' + totalCls + '">' + totalText + '</td>' +
      '</tr>'
    );
  }).join('');
  ...
  if (!State.finalisasiDone) {
    ...
    confirmHTML =
      ...
      (allFilled && !overBudget ? '' : 'disabled') + '>&#10003; Konfirmasi Rancangan Anggaran</button>' +
      '</div>';
    if (overBudget) {
      confirmHTML += '<p style="font-size:0.85rem;color:var(--color-error-strong);margin-top:var(--space-2);">Kurangi anggaran agar tidak melebihi Rp 600.000.</p>';
    } else if (!allFilled) {
      confirmHTML += '<p style="font-size:0.85rem;color:var(--color-ink-muted);margin-top:var(--space-2);">Isi semua kolom terlebih dahulu.</p>';
    }
  } else {
    confirmHTML = buildFeedbackBox('success', '&#127775;',
      '<strong>Rancangan anggaran telah dikonfirmasi!</strong> Total anggaran: <strong>' + formatRupiah(grand) + '</strong> dari batas Rp 600.000.' +
      '<div class="btn-group" style="margin-top:var(--space-3);">' +
  ...
  container.innerHTML =
    ...
    '<td class="grand-total-val" id="grandTotalCell">' + (grand > 0 ? formatRupiah(grand) : '—') + '</td>' +
    '</tr></tfoot>' +
    '</table></div>' +

    '<div class="budget-bar-wrap">' +
    '<div class="budget-bar-labels">' +
    '<span class="budget-bar-labels__used">Terpakai: ' + formatRupiah(grand) + ' (' + pct + '%)</span>' +
    '<span class="' + sisaClass + '">' + sisaText + '</span>' +
    '</div>' +
    '<div class="budget-bar" aria-label="Penggunaan anggaran" role="progressbar" ' +
    'aria-valuenow="' + pct + '" aria-valuemin="0" aria-valuemax="100">' +
    '<div class="budget-bar__fill ' + barClass + '" id="budgetBarFill" style="width:' + pct + '%"></div>' +
    '</div></div>' +
...
function updateFinalisasiLive(container) {
  ...
  if (labelsWrap) {
    ...
    var remainEl = labelsWrap.querySelector('.budget-bar-labels__remaining, .budget-bar-labels__remaining--over');
    if (usedEl) usedEl.textContent = 'Terpakai: ' + formatRupiah(grand) + ' (' + pct + '%)';
    if (remainEl) {
      remainEl.className = overBudget ? 'budget-bar-labels__remaining--over' : 'budget-bar-labels__remaining';
...
function renderPresentasiRancangan(container) {
  ...
  /* compute from state */
  var grand = calcFinalisasiTotals();
  var konsumsiTotal = State.finalisasiItems && State.finalisasiItems[1]
    ? (State.finalisasiItems[1].total || 0) : 0;

  var pctExpected = Math.round(grand / DATA.finalisasiRancangan.batas * 100);
  var iuranExpected = Math.round(konsumsiTotal / 35);

  /* budget summary */
  var summaryRowsHTML = DATA.finalisasiRancangan.kategori.map(function (kat, i) {
    var item = State.finalisasiItems ? State.finalisasiItems[i] : { qty: '—', harga: '—', total: 0 };
    return (
      '<div class="budget-summary__row">' +
      '<span class="budget-summary__cat">' + kat.icon + ' ' + esc(kat.nama) + '</span>' +
      '<span class="budget-summary__detail">' + esc(item.qty) + ' × ' + formatRupiah(item.harga || 0) + '</span>' +
      '<span class="budget-summary__subtotal">' + formatRupiah(item.total || 0) + '</span>' +
      '</div>'
    );
  }).join('');
  ...
  var soal1FB = '';
  if (ans1.done) {
    soal1FB = buildFeedbackBox('success', '&#10003;',
  ...
  var soal2FB = '';
  if (ans2.done) {
    soal2FB = buildFeedbackBox('success', '&#10003;',
      'Benar! ' + formatRupiah(konsumsiTotal) + ' ÷ 35 ≈ <strong>' + formatRupiah(iuranExpected) + '</strong> per siswa'
    );
  }
  ...
  container.innerHTML =
    ...
    '<span class="budget-summary__total">' + formatRupiah(grand) + '</span>' +
    '</div>' +
    summaryRowsHTML +
    '</div>' +
    ...
    '<p style="font-size:0.85rem;color:var(--color-ink-muted);margin-bottom:var(--space-2);">' + esc(kata.instruction) + '</p>' +
    '<textarea id="narasiInput" class="narasi-field" ' +
    'placeholder="' + esc(kata.narasi_placeholder) + '" ' +
    'aria-label="Narasi justifikasi anggaran">' + esc(State.presentasiNarasi) + '</textarea>' +
    '<div class="char-count' + (narasiOk ? ' char-count--ok' : '') + '" id="narasiCount">' +
    narasiLen + ' karakter' + (narasiOk ? ' &#10003;' : ' (minimal 30 karakter)') + '</div>' +
    '</div>' +

    /* soal 1 */
    '<div class="panel panel--compact" style="margin-top:var(--space-4);">' +
    '<h4>' + kata.soal[0].label + '</h4>' +
    '<p>' + kata.soal[0].question + '</p>' +
    '<p style="font-size:0.82rem;color:var(--color-ink-muted);">Batas anggaran: Rp 600.000 | Anggaranmu: ' + formatRupiah(grand) + '</p>' +
    '<div class="input-row" style="margin:var(--space-3) 0;">' +
    '<input type="text" inputmode="numeric" id="persen1Input" class="input-text" ' +
    'value="' + esc(ans1.input) + '" ' +
    (ans1.done ? 'disabled ' : '') +
    'placeholder="Masukkan % (contoh: 90)" aria-label="Jawaban persentase" />' +
    '<button type="button" class="btn btn--primary" id="persen1CheckBtn" ' + (ans1.done ? 'disabled' : '') + '>Periksa</button>' +
    '</div>' +
    (soal1FB || '<div id="persen1Feedback"></div>') +
    '<p style="font-size:0.82rem;color:var(--color-ink-muted);margin-top:var(--space-2);">&#128161; ' + esc(kata.soal[0].hint) + '</p>' +
    '</div>' +

    /* soal 2 */
    '<div class="panel panel--compact" style="margin-top:var(--space-3);">' +
    '<h4>' + kata.soal[1].label + '</h4>' +
    '<p>' + kata.soal[1].question + '</p>' +
    '<p style="font-size:0.82rem;color:var(--color-ink-muted);">Anggaran konsumsimu: ' + formatRupiah(konsumsiTotal) + '</p>' +
    '<div class="input-row" style="margin:var(--space-3) 0;">' +
    '<input type="text" inputmode="numeric" id="iuran2Input" class="input-text" ' +
    'value="' + esc(ans2.input) + '" ' +
    (ans2.done ? 'disabled ' : '') +
    'placeholder="Iuran per siswa (Rp)" aria-label="Jawaban iuran per siswa" />' +
    '<button type="button" class="btn btn--primary" id="iuran2CheckBtn" ' + (ans2.done ? 'disabled' : '') + '>Periksa</button>' +
    '</div>' +
    (soal2FB || '<div id="iuran2Feedback"></div>') +
    '<p style="font-size:0.82rem;color:var(--color-ink-muted);margin-top:var(--space-2);">&#128161; ' + esc(kata.soal[1].hint) + '</p>' +
    '</div>' +
    ...
    'placeholder="' + esc(kata.refleksi_placeholder) + '" ' +
    'aria-label="Refleksi rasionalitas anggaran">' + esc(State.presentasiRefleksi) + '</textarea>' +
    '<div class="char-count' + (refleksiOk ? ' char-count--ok' : '') + '" id="refleksiCount">' +
    refleksiLen + ' karakter' + (refleksiOk ? ' &#10003;' : ' (minimal 30 karakter)') + '</div>' +
    '</div>' +
    ...
    (canComplete ? '' : 'disabled') + '>Selesai Presentasi →</button>' +
    '</div>' +
    '</div></section>';
  ...
  if (narasiInp) {
    narasiInp.addEventListener('input', function () {
      ...
      if (lanjutBtn2) {
        lanjutBtn2.disabled = !(ok && State.presentasiAnswers[0].done && State.presentasiAnswers[1].done && State.presentasiRefleksi.length >= 30);
      }
    });
  }
  ...
  if (reflInp) {
    reflInp.addEventListener('input', function () {
      ...
      var lanjutBtn2 = document.getElementById('presentasiLanjutBtn');
      if (lanjutBtn2) {
        lanjutBtn2.disabled = !(State.presentasiNarasi.length >= 30 && State.presentasiAnswers[0].done && State.presentasiAnswers[1].done && ok);
      }
    });
  }
  ...
  if (p1Btn) {
    ...
    if (p1Inp) p1Inp.addEventListener('keydown', function (e) { if (e.key === 'Enter') p1Btn.click(); });
  }
  ...
  if (p2Btn) {
    p2Btn.addEventListener('click', function () {
      ...
      if (parsed2.error === 'empty') {
        if (fb2) fb2.innerHTML = buildFeedbackBox('error', '&#9888;', 'Masukkan jawaban terlebih dahulu.');
        return;
      }
      if (parsed2.error === 'invalid' || Math.abs(parsed2.value - iuranExpected) > 500) {
        if (fb2) fb2.innerHTML = buildFeedbackBox('error', '&#10007;',
    ...
    if (p2Inp) p2Inp.addEventListener('keydown', function (e) { if (e.key === 'Enter') p2Btn.click(); });
  }
...
function renderRefleksi(container) {
  ...
  var soalHTML = reflData.soal.map(function (s, i) {
    var val = answers[s.id] || '';
    return (
      '<div class="panel panel--compact" style="margin-bottom:var(--space-3);">' +
      '<label for="refl_' + s.id + '" style="display:block;font-weight:600;margin-bottom:var(--space-2);">' +
      (i + 1) + '. ' + s.question + '</label>' +
      '<textarea id="refl_' + s.id + '" class="narasi-field" style="min-height:90px;" ' +
      'placeholder="' + esc(s.placeholder) + '" ' +
      'data-ref-id="' + s.id + '" ' +
      'aria-label="Jawaban pertanyaan refleksi ' + (i + 1) + '">' + esc(val) + '</textarea>' +
      '</div>'
    );
  }).join('');
  ...
  container.innerHTML =
    ...
    '<div class="panel">' +
    '<p style="font-size:0.88rem;color:var(--color-ink-muted);margin-bottom:var(--space-4);" ' +
    'id="reflNote">' + reflData.note + '</p>' +

    soalHTML +

    (State.refleksiSaved
      ? buildFeedbackBox('success', '&#128218;', 'Refleksi tersimpan! Klik "Selesai" untuk menyelesaikan pembelajaran.')
    ...
    (allFilled ? '' : 'disabled') + '>&#128190; Simpan Refleksi</button>' +
    '<button type="button" class="btn btn--primary" id="refleksiSelesaiBtn" ' +
    (State.refleksiSaved ? '' : 'disabled') + '>Selesai →</button>' +
    '</div></div></section>';
...
function renderSelesai(container) {
  ...
  var summaryRowsHTML = DATA.finalisasiRancangan.kategori.map(function (kat, i) {
    ...
    return (
      '<div class="budget-summary__row">' +
      '<span class="budget-summary__cat">' + kat.icon + ' ' + esc(kat.nama) + '</span>' +
      '<span class="budget-summary__detail">' + pctCat + '% dari total</span>' +
      '<span class="budget-summary__subtotal">' + formatRupiah(item.total || 0) + '</span>' +
      '</div>'
    );
  }).join('');
  ...
  var opsHTML = operasiList.map(function (op) {
    return (
      '<div style="display:flex;align-items:center;gap:var(--space-3);padding:var(--space-2) 0;' +
      'border-bottom:1px dashed var(--color-border);">' +
      '<span style="font-size:1.2rem;min-width:24px;text-align:center;">' + op.icon + '</span>' +
      '<div><strong style="font-size:0.9rem;">' + esc(op.label) + '</strong>' +
      '<div style="font-size:0.8rem;color:var(--color-ink-muted);">' + esc(op.contoh) + '</div></div>' +
      '</div>'
    );
  }).join('');
  ...
  container.innerHTML =
    ...
    '<span></span>' +
    '<span class="budget-summary__subtotal" style="color:var(--color-success-strong);">' + formatRupiah(sisa) + '</span>' +
    '</div></div>' +
    '</div>' +
...
function buildNavRow(currentIdx, total, allDone, nextStage) {
  ...
  return (
    '<div class="btn-group btn-group--spread" style="margin-top:var(--space-4);">' +
    '<button type="button" class="btn btn--ghost" id="navPrevBtn" ' + prevDisabled + '>← Soal Sebelumnya</button>' +
    '<button type="button" class="btn btn--ghost" id="navNextBtn" ' + nextDisabled + '>Soal Berikutnya →</button>' +
    '</div>' +
    (lanjutHTML
      ? '<div class="btn-group btn-group--end" style="margin-top:var(--space-3);">' + lanjutHTML + '</div>'
      : ''
    )
  );
}

function attachNavRowListeners(container, currentIdx, total, stateIdxKey, renderFn, stageId, nextStage) {
