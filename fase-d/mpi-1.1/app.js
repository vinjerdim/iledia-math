...
function initExerciseArrays() {
  if (!State.kenaliSeen || State.kenaliSeen.length !== DATA.kenaliBulat.konteks.length) {
    State.kenaliSeen = DATA.kenaliBulat.konteks.map(function () { return false; });
  }
  if (!State.garisBilanganExercises || State.garisBilanganExercises.length !== DATA.garisBilangan.soal.length) {
  ...
  if (!State.membandingkanExercises || State.membandingkanExercises.length !== DATA.membandingkan.soal.length) {
  ...
  if (!State.mengurutkanExercises || State.mengurutkanExercises.length !== DATA.mengurutkan.soal.length) {
    State.mengurutkanExercises = DATA.mengurutkan.soal.map(function () {
      return { correct: false, order: [], revealed: false };
    });
  }
  if (!State.situasiNyataExercises || State.situasiNyataExercises.length !== DATA.situasiNyata.soal.length) {
...
function buildNumberLineSVG(value, min, max) {
  ...
  for (var v = min; v <= max; v++) {
    ...
    ticks += '<line class="nl-tick' + (isMajor ? ' nl-tick--major' : '') + '" x1="' + x + '" y1="' + (axisY - h / 2) + '" x2="' + x + '" y2="' + (axisY + h / 2) + '"/>';
    if (isMajor || v === 0) {
      var lCls = v === 0 ? 'nl-label nl-label--zero' : 'nl-label';
      labels += '<text class="' + lCls + '" x="' + x + '" y="' + (axisY + h / 2 + 4) + '">' + v + '</text>';
    }
  }
  ...
  var isNeg = value < 0;
  var ptLabel =
    '<text class="nl-point-label" x="' + px + '" y="' + (axisY - 14) + '">' +
  ...
  var arrowHead =
    '<polygon class="nl-arrow" points="' + arrowR + ',' + axisY + ' ' + (arrowR - 8) + ',' + (axisY - 4) + ' ' + (arrowR - 8) + ',' + (axisY + 4) + '"/>' +
    '<polygon class="nl-arrow" points="' + arrowL + ',' + axisY + ' ' + (arrowL + 8) + ',' + (axisY - 4) + ' ' + (arrowL + 8) + ',' + (axisY + 4) + '"/>';

  return (
    '<svg class="numberline-svg" viewBox="0 0 ' + W + ' ' + H + '" xmlns="http://www.w3.org/2000/svg" aria-label="Garis bilangan dari ' + min + ' hingga ' + max + '">' +
    '<line class="nl-axis" x1="' + (padX - 10) + '" y1="' + axisY + '" x2="' + (W - padX + 10) + '" y2="' + axisY + '"/>' +
    ticks + labels + point + ptLabel + arrowHead +
    '</svg>'
  );
}
...
function renderCurrentStage() {
  ...
  switch (State.currentStage) {
    ...
    case 'kenaliBulat':     renderKenaliBulat(container); break;
    case 'garisBilangan':   renderGarisBilangan(container); break;
    case 'membandingkan':   renderMembandingkan(container); break;
    case 'mengurutkan':     renderMengurutkan(container); break;
    case 'situasiNyata':    renderSituasiNyata(container); break;
...
function renderKenaliBulat(container) {
  ...
  var tabsHTML = ctxList.map(function (c, i) {
    ...
    return (
      '<button type="button" class="stage-nav__item' + done + '"' + active + ' data-ctx-idx="' + i + '">' +
      '<span class="stage-nav__num">' + (State.kenaliSeen[i] ? '&#10003;' : (i + 1)) + '</span>' +
      esc(c.badge) + '</button>'
    );
  }).join('');
  ...
  var itemsHTML = ctx.items.map(function (item) {
    ...
    return (
      '<div class="int-item ' + colorCls + '">' +
      '<div class="int-item__value">' + (item.value >= 0 ? (item.value === 0 ? '0' : '+' + item.value) : item.value) + '</div>' +
      '<div class="int-item__info">' +
      '<div class="int-item__label">' + esc(item.label) + '</div>' +
      '<div class="int-item__note">' + esc(item.note) + '</div>' +
      '</div>' +
  ...
  container.innerHTML =
    ...
    '<div class="panel">' +
    '<p style="font-size:0.88rem;color:var(--color-ink-muted);margin-bottom:var(--space-3);">' + esc(DATA.kenaliBulat.instruction) + '</p>' +
    ...
    '<span style="font-size:2rem;">' + ctx.icon + '</span>' +
    '<div>' +
    '<span class="badge-chip">' + esc(ctx.badge) + '</span>' +
    '<p style="margin:var(--space-1) 0 0;font-size:0.92rem;">' + esc(ctx.story) + '</p>' +
    '</div></div>' +
    '<div class="int-item-list">' + itemsHTML + '</div>' +
    '</div>' +

    '<div class="insight-box">' +
    '<span class="insight-box__icon">💡</span>' +
    '<div>' + esc(ctx.insight) + '</div>' +
    '</div>' +
    '</div>' +
    ...
    (allSeen
      ? '<div class="btn-group btn-group--end">' +
        ...
        '</div>'
      : '<p style="text-align:center;font-size:0.85rem;color:var(--color-ink-muted);margin-top:var(--space-3);">Jelajahi semua ' + ctxList.length + ' konteks untuk melanjutkan.</p>') +
    '</section>';
...
function renderGarisBilangan(container) {
  ...
  var s = soal[idx];
  var ex = exArr[idx];
  var allDone = exArr.filter(function (e) { return e.correct; }).length === soal.length;

  var statuses = exArr.map(function (e) { return e.correct ? 'correct' : (e.attempts > 0 ? 'incorrect' : null); });
  ...
  if (ex.correct) {
    ...
    feedbackHTML = buildFeedbackBox('info', '👁', '<strong>Jawaban:</strong> ' + s.nilai + '. ' + s.explanation);
  } else if (ex.hintShown) {
    feedbackHTML = buildFeedbackBox('warning', '💡', '<strong>Petunjuk:</strong> ' + s.hint);
  } else if (ex.attempts > 0) {
    feedbackHTML = buildFeedbackBox('error', '✗', 'Jawabanmu <strong>' + esc(ex.userInput) + '</strong> belum tepat. Coba lagi atau lihat petunjuk.');
  }
  ...
  if (!ex.correct && !ex.revealed) {
    actionHTML =
      ...
      '<input type="text" inputmode="numeric" id="gbInput" class="input-text" placeholder="..." aria-label="Nilai bilangan pada titik merah" value="' + esc(ex.userInput) + '">' +
  ...
  if (ex.correct || ex.revealed) {
    if (idx < soal.length - 1) {
      navHTML = '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary" id="gbNextBtn">Soal Berikutnya →</button></div>';
    } else if (allDone || exArr.filter(function (e) { return e.correct || e.revealed; }).length === soal.length) {
      navHTML = '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary btn--large" id="gbFinishBtn">Lanjut: Membandingkan →</button></div>';
    }
  }
  ...
  container.innerHTML =
    ...
    '</div>' +
    '<div class="panel">' +
    '<p style="font-size:0.88rem;color:var(--color-ink-muted);margin-bottom:var(--space-3);">' + esc(DATA.garisBilangan.instruction) + '</p>' +
    dotsHTML +
    ...
    '<div class="numberline-wrap">' + svgHTML + '</div>' +
    actionHTML +
  ...
  if (inp) inp.addEventListener('keydown', function (e) { if (e.key === 'Enter' && checkBtn) checkBtn.click(); });

  if (checkBtn) checkBtn.addEventListener('click', function () {
    if (!inp) return;
    var val = inp.value;
    var parsed = parseInputInt(val);
    if (parsed.error === 'empty') { showNotice('Masukkan bilangan terlebih dahulu.'); return; }
    if (parsed.error === 'invalid') { showNotice('Masukkan bilangan bulat yang valid (contoh: −3, 0, 7).'); return; }
    ex.userInput = val;
    ex.attempts += 1;
...
function renderMembandingkan(container) {
  ...
  var s = soal[idx];
  var ex = exArr[idx];
  var allAnswered = exArr.filter(function (e) { return e.correct || e.attempts >= 2; }).length === soal.length;
  ...
  else if (ex.attempts > 0 && !ex.correct && ex.chosen) symbolCls += ' compare-symbol-display--incorrect';
  ...
  if (ex.correct) {
    ...
    feedbackHTML = buildFeedbackBox('error', '✗', 'Simbol <strong>' + esc(ex.chosen) + '</strong> belum tepat. Ingat: pada garis bilangan, bilangan di sebelah kiri selalu lebih kecil.');
    if (ex.attempts >= 2) {
      feedbackHTML = buildFeedbackBox('info', '👁', '<strong>Jawaban:</strong> ' + s.a + ' ' + s.answer + ' ' + s.b + '. ' + s.explanation);
    }
  }
  ...
  if (ex.correct || ex.attempts >= 2) {
    if (idx < soal.length - 1) {
      navHTML = '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary" id="mbNextBtn">Soal Berikutnya →</button></div>';
    } else if (allAnswered) {
      navHTML = '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary btn--large" id="mbFinishBtn">Lanjut: Mengurutkan →</button></div>';
    }
  }
  ...
  container.innerHTML =
    ...
    '</div>' +
    '<div class="panel">' +
    '<p style="font-size:0.88rem;color:var(--color-ink-muted);margin-bottom:var(--space-3);">' + esc(DATA.membandingkan.instruction) + '</p>' +
    dotsHTML +
    '<div class="compare-card">' +
    '<div class="' + aClass + '">' + s.a + '</div>' +
    '<div class="compare-symbol-area">' +
    '<div class="' + symbolCls + '" aria-label="simbol perbandingan">' + esc(symbolDisplay) + '</div>' +
    '<div class="compare-btn-group">' +
    '<button type="button" class="compare-btn" id="btnLt"' + btnDisabled + ' aria-label="kurang dari">&lt;</button>' +
    '<button type="button" class="compare-btn" id="btnEq"' + btnDisabled + ' aria-label="sama dengan">=</button>' +
    '<button type="button" class="compare-btn" id="btnGt"' + btnDisabled + ' aria-label="lebih dari">&gt;</button>' +
    '</div>' +
    '</div>' +
    '<div class="' + bClass + '">' + s.b + '</div>' +
    '</div>' +
    (feedbackHTML ? '<div style="margin-top:var(--space-3);">' + feedbackHTML + '</div>' : '') +
  ...
  if (btnLt) btnLt.addEventListener('click', function () { handleChoice('<'); });
  if (btnEq) btnEq.addEventListener('click', function () { handleChoice('='); });
  if (btnGt) btnGt.addEventListener('click', function () { handleChoice('>'); });
...
function renderMengurutkan(container) {
  ...
  var s = soal[idx];
  var ex = exArr[idx];
  var allDone = exArr.filter(function (e) { return e.correct || e.revealed; }).length === soal.length;

  /* Dots */
  var statuses = exArr.map(function (e) { return e.correct ? 'correct' : (e.revealed ? 'incorrect' : null); });
  var dotsHTML = buildProgressDots(soal.length, idx, statuses);

  /* Current order in answer slots */
  var currentOrder = ex.order || [];
  /* Remaining chips (not yet placed) */
  var remaining = s.acak.filter(function (v) { return currentOrder.indexOf(v) === -1; });

  /* Chips pool HTML */
  var poolHTML = s.acak.map(function (v) {
    var placed = currentOrder.indexOf(v) !== -1;
    var isNeg = v < 0;
    var chipCls = 'sort-chip' + (isNeg ? ' sort-chip--negative' : '') + (placed ? ' sort-chip--selected' : '');
    return (
      '<button type="button" class="' + chipCls + '" data-chip-val="' + v + '"' + (placed || ex.correct || ex.revealed ? ' disabled' : '') + '>' +
      v +
      '</button>'
    );
  }).join('');
  ...
  for (var i = 0; i < s.angka.length; i++) {
    ...
    if (filled) slotCls += isNegSlot ? ' sort-slot--filled sort-slot--filled-negative' : ' sort-slot--filled';
    slotsHTML +=
      '<div class="' + slotCls + '" data-slot-idx="' + i + '">' +
      '<span class="sort-slot__num">' + (i + 1) + '</span>' +
      (filled ? String(val) : '&nbsp;') +
      '</div>';
  }
  ...
  var feedbackHTML = '';
  if (ex.correct) {
    feedbackHTML = buildFeedbackBox('success', '✓', '<strong>Urutan benar!</strong> ' + s.explanation);
  } else if (ex.revealed) {
    feedbackHTML = buildFeedbackBox('info', '👁', '<strong>Urutan yang benar:</strong> ' + s.angka.join(', ') + '. ' + s.explanation);
  }

  var progressText = currentOrder.length === 0
  ...
  if (!ex.correct && !ex.revealed && currentOrder.length === s.angka.length) {
    checkBtnHTML = '<button type="button" class="btn btn--primary" id="sortCheckBtn">Periksa Urutan</button>';
  }
  var resetBtnHTML = '';
  if (!ex.correct && !ex.revealed && currentOrder.length > 0) {
    resetBtnHTML = '<button type="button" class="btn btn--ghost btn--small" id="sortResetBtn">↺ Mulai Ulang</button>';
  }
  var revealBtnHTML = '';
  if (!ex.correct && !ex.revealed && (ex.revealed === false)) {
    revealBtnHTML = '<button type="button" class="btn btn--ghost btn--small" id="sortRevealBtn">👁 Lihat Jawaban</button>';
  }

  var navHTML = '';
  if (ex.correct || ex.revealed) {
    if (idx < soal.length - 1) {
      navHTML = '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary" id="sortNextBtn">Soal Berikutnya →</button></div>';
    } else if (allDone) {
      navHTML = '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary btn--large" id="sortFinishBtn">Lanjut: Situasi Nyata →</button></div>';
    }
  }
  ...
  container.innerHTML =
    ...
    '</div>' +
    '<div class="panel">' +
    '<p style="font-size:0.88rem;color:var(--color-ink-muted);margin-bottom:var(--space-3);">' + esc(DATA.mengurutkan.instruction) + '</p>' +
    dotsHTML +
    '<div class="panel panel--compact panel--hero" style="margin-bottom:var(--space-3);">' +
    '<span class="badge-chip">' + esc(s.badge) + '</span>' +
    '<p style="margin:var(--space-2) 0 0;font-size:0.92rem;">' + esc(s.story) + '</p>' +
    '<div style="display:flex;flex-wrap:wrap;gap:var(--space-2);margin-top:var(--space-3);">' +
    s.acak.map(function (v) {
      return '<span style="font-family:var(--font-mono);font-size:1.05rem;font-weight:700;color:var(--color-primary-strong);">' + v + (s.unit ? ' ' + s.unit : '') + '</span>';
    ...
    '<div class="sort-chips-pool" id="sortPool">' + poolHTML + '</div>' +

    '<p style="font-size:0.85rem;font-weight:600;margin-bottom:var(--space-2);">Urutan dari terkecil ke terbesar:</p>' +
    '<div class="sort-answer-slots" id="sortSlots">' + slotsHTML + '</div>' +
    '<p class="sort-progress-text">' + esc(progressText) + '</p>' +

    (feedbackHTML ? '<div style="margin-top:var(--space-3);">' + feedbackHTML + '</div>' : '') +

    '<div class="btn-group" style="margin-top:var(--space-3);">' +
    resetBtnHTML + revealBtnHTML + checkBtnHTML +
    '</div>' +
    '</div>' +
    navHTML +
    '</section>';
  ...
  if (checkBtn) checkBtn.addEventListener('click', function () {
    var correct = true;
    for (var i = 0; i < s.angka.length; i++) {
      if (ex.order[i] !== s.angka[i]) { correct = false; break; }
    }
    ex.correct = correct;
...
function renderSituasiNyata(container) {
  ...
  var s = soal[idx];
  var ex = exArr[idx];
  var allAnswered = exArr.filter(function (e) { return e.correct || e.checked; }).length === soal.length;

  var statuses = exArr.map(function (e) { return e.correct ? 'correct' : (e.checked ? 'incorrect' : null); });
  var dotsHTML = buildProgressDots(soal.length, idx, statuses);

  var letters = ['A', 'B', 'C', 'D'];
  var choicesHTML = s.options.map(function (opt, i) {
    var cls = 'choice-btn';
    if (ex.checked) {
    ...
    return (
      '<button type="button" class="' + cls + '" data-opt-id="' + opt.id + '">' +
  ...
  if (!ex.checked && ex.attempts > 0) {
    hintHTML = '<div style="margin-top:var(--space-3);">' + buildFeedbackBox('warning', '💡', '<strong>Petunjuk:</strong> ' + s.hint) + '</div>';
  }

  var navHTML = '';
  if (ex.correct || ex.checked) {
    if (idx < soal.length - 1) {
      navHTML = '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary" id="snNextBtn">Soal Berikutnya →</button></div>';
    } else if (allAnswered) {
      navHTML = '<div class="btn-group btn-group--end"><button type="button" class="btn btn--primary btn--large" id="snFinishBtn">Lanjut: Refleksi →</button></div>';
    }
  }
  ...
  container.innerHTML =
    ...
    '</div>' +
    '<div class="panel">' +
    '<p style="font-size:0.88rem;color:var(--color-ink-muted);margin-bottom:var(--space-3);">' + esc(DATA.situasiNyata.instruction) + '</p>' +
    dotsHTML +
    '<div class="panel panel--hero" style="margin-bottom:var(--space-3);">' +
    '<div style="display:flex;align-items:center;gap:var(--space-3);flex-wrap:wrap;margin-bottom:var(--space-3);">' +
    ...
    '<span class="badge-chip">' + esc(s.badge) + '</span>' +
    '</div>' +
    '<p style="font-size:0.95rem;">' + s.story + '</p>' +
    '<p style="font-weight:600;font-size:1rem;margin-bottom:0;">' + s.question + '</p>' +
    '</div>' +
    '<div class="choice-list" id="snChoices">' + choicesHTML + '</div>' +
    (feedbackHTML ? '<div style="margin-top:var(--space-3);">' + feedbackHTML + '</div>' : '') +
    hintHTML +
    '</div>' +
    navHTML +
...
function renderRefleksi(container) {
  ...
  var fieldsHTML = soal.map(function (s, i) {
    var saved = State.refleksiAnswers[s.id] || '';
    return (
      '<div class="field-group">' +
      '<label for="refl_' + s.id + '">' + (i + 1) + '. ' + s.question + '</label>' +
      '<textarea id="refl_' + s.id + '" class="input-textarea" placeholder="' + esc(s.placeholder) + '" rows="3">' + esc(saved) + '</textarea>' +
      '</div>'
    );
  }).join('');
  ...
  container.innerHTML =
    ...
    '</div>' +
    '<div class="panel">' +
    '<p style="font-size:0.9rem;color:var(--color-ink-muted);margin-bottom:var(--space-4);">' + DATA.refleksi.note + '</p>' +
    fieldsHTML +
    (State.refleksiSaved
      ? buildFeedbackBox('success', '✓', 'Refleksimu sudah disimpan. Klik tombol di bawah untuk melanjutkan.')
      : '') +
    '<div class="btn-group btn-group--end">' +
    '<button type="button" class="btn btn--ghost" id="saveReflBtn">Simpan Refleksi</button>' +
    '<button type="button" class="btn btn--primary btn--large" id="finishReflBtn"' + (State.refleksiSaved ? '' : ' disabled') + '>Selesai ✓</button>' +
    '</div></div></section>';
...
function renderSelesai(container) {
  var gbDone = State.garisBilanganExercises.filter(function (e) { return e.correct; }).length;
  var gbTotal = DATA.garisBilangan.soal.length;
  var mbDone = State.membandingkanExercises.filter(function (e) { return e.correct; }).length;
  var mbTotal = DATA.membandingkan.soal.length;
  var urtDone = State.mengurutkanExercises.filter(function (e) { return e.correct; }).length;
  var urtTotal = DATA.mengurutkan.soal.length;
  var snDone = State.situasiNyataExercises.filter(function (e) { return e.correct; }).length;
  ...
  container.innerHTML =
    ...
    '<div class="achievement-card"><div class="achievement-card__icon">📏</div><div class="achievement-card__title">Garis Bilangan</div><div class="achievement-card__desc">' + gbDone + '/' + gbTotal + ' soal benar</div></div>' +
    '<div class="achievement-card"><div class="achievement-card__icon">⚖️</div><div class="achievement-card__title">Membandingkan</div><div class="achievement-card__desc">' + mbDone + '/' + mbTotal + ' soal benar</div></div>' +
    '<div class="achievement-card"><div class="achievement-card__icon">🔢</div><div class="achievement-card__title">Mengurutkan</div><div class="achievement-card__desc">' + urtDone + '/' + urtTotal + ' soal benar</div></div>' +
    '<div class="achievement-card"><div class="achievement-card__icon">🌍</div><div class="achievement-card__title">Situasi Nyata</div><div class="achievement-card__desc">' + snDone + '/' + snTotal + ' soal benar</div></div>' +
    '</div></div>' +
