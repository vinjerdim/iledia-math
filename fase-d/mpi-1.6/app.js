...
function calcTotalBelanja(items) {
    return items.reduce(function (s, it) { return s + it.harga; }, 0);
}
...
var STAGE_LABELS = ['Orientasi', 'Pembulatan', 'Estimasi Total', 'Decision Challenge', 'Asesmen', 'Refleksi', 'Selesai'];
...
function saveState() {
    ...
    } catch (e) { /* simpan gagal — lanjutkan */ }
}
...
function buildFeedbackBox(type, icon, html) {
    return '<div class="feedback-box feedback-box--' + esc(type) + '" role="alert" aria-live="polite">' +
...
function buildProgressDots(total, currentIdx, results) {
    ...
    for (var i = 0; i < total; i++) {
        ...
        var label = 'Soal ' + (i + 1);
        if (i === currentIdx) {
            cls += ' exercise-progress__dot--current'; label += ' (aktif)';
        } else if (results[i] && results[i].chosen !== null) {
            if (results[i].isCorrect) { cls += ' exercise-progress__dot--correct'; label += ' ✓'; }
            else { cls += ' exercise-progress__dot--incorrect'; label += ' ✗'; }
        }
        dots += '<span class="' + cls + '" aria-label="' + esc(label) + '">' + (i + 1) + '</span>';
    }
    return '<div class="exercise-progress" aria-label="Progress soal">' + dots +
        '<span class="exercise-label">Soal ' + (currentIdx + 1) + ' dari ' + total + '</span>' +
        '</div>';
}
...
function buildNumberLine(harga, level, correct) {
    ...
    return '<div class="number-line">' +
        '<p class="number-line__title">Posisi ' + formatRupiah(harga) + ' pada garis bilangan</p>' +
        ...
        '<div class="number-line__fill-under" style="left:' + fillLeft.toFixed(1) + '%;width:' + fillWidth.toFixed(1) + '%" aria-hidden="true"></div>' +
        '<div class="number-line__value-dot" style="left:' + pct.toFixed(1) + '%">' +
        '<span class="number-line__value-label">' + formatRupiah(harga) + '</span>' +
        '</div>' +
        '</div>' +
        '<div class="number-line__labels">' +
        '<span>' + formatRupiah(lower) + '</span>' +
        '<span style="color:var(--color-ink-muted);">tengah: ' + formatRupiah(midpoint) + '</span>' +
        '<span>' + formatRupiah(upper) + '</span>' +
        '</div>' +
        '<p class="number-line__conclusion">' + direction + '</p>' +
        '</div>';
}
...
function renderCurrentStage() {
    ...
    switch (State.currentStage) {
        ...
        case 'pembulatan': renderPembulatan(container); break;
...
function renderOrientasi(container) {
    container.innerHTML =
        ...
        '<p class="stage-head__goal">' + esc(DATA.meta.goal) + '</p>' +
        '</div>' +
...
function renderPembulatan(container) {
    ...
    container.innerHTML =
        ...
        '<p class="stage-head__goal">' + esc(DATA.pembulatan.instruction) + '</p>' +
        '</div>' +

        buildProgressDots(soalList.length, idx, State.pb.results) +

        '<div class="panel">' +
        '<p style="font-size:0.85rem;color:var(--color-ink-muted);margin-bottom:var(--space-3);">Bulatkan harga berikut ke <strong>' + esc(soal.levelLabel) + '</strong>:</p>' +
        '<p style="font-family:var(--font-mono);font-size:2rem;font-weight:700;color:var(--color-orange-strong);margin:0 0 var(--space-4);" aria-label="Harga: ' + formatRupiah(soal.harga) + '">' + formatRupiah(soal.harga) + '</p>' +
        ...
        displayOpts.map(function (opt) {
            ...
            return '<button type="button" class="' + cls + '"' +
                (isAnswered ? ' disabled' : '') +
                ' data-opt="' + opt + '"' +
                ' aria-label="' + formatRupiah(opt) + '">' +
                formatRupiah(opt) + '</button>';
        }).join('') +
        '</div>' +
        ...
        (isAnswered
            ? buildFeedbackBox(
                ...
                (result.isCorrect
                    ...
                    : '<strong>Belum tepat.</strong> Jawaban yang benar adalah <strong>' + formatRupiah(soal.correct) + '</strong>. ' + esc(soal.explanation)) +
                buildNumberLine(soal.harga, soal.level, soal.correct)
            )
            : '<details class="hint-reveal"><summary></summary>' +
            '<div class="hint-reveal__content">' + esc(soal.hint) + '</div></details>'
        ) +

        '<div class="btn-group btn-group--' + (idx > 0 ? 'spread' : 'end') + '" style="margin-top:var(--space-4);">' +
        (idx > 0 ? '<button type="button" class="btn btn--ghost" id="pbPrevBtn">← Sebelumnya</button>' : '') +
        (isAnswered
...
function renderPembulatanSummary(container) {
    ...
    var rows = soalList.map(function (soal, i) {
        var r = State.pb.results[i];
        var ok = r && r.isCorrect;
        return '<tr>' +
            '<td style="padding:var(--space-2) var(--space-3);">' + formatRupiah(soal.harga) + '</td>' +
            '<td style="padding:var(--space-2) var(--space-3);color:var(--color-ink-muted);font-size:0.82rem;">' + esc(soal.levelLabel) + '</td>' +
            '<td style="padding:var(--space-2) var(--space-3);font-family:var(--font-mono);">' + formatRupiah(soal.correct) + '</td>' +
            '<td style="padding:var(--space-2) var(--space-3);text-align:center;">' +
            (ok ? '<span style="color:var(--color-success-strong);font-weight:700;">✓</span>' : '<span style="color:var(--color-error-strong);font-weight:700;">✗</span>') +
            '</td>' +
            '</tr>';
    }).join('');
    ...
    container.innerHTML =
        ...
        buildFeedbackBox(feedbackType, feedbackType === 'success' ? '🎉' : feedbackType === 'info' ? '👍' : '💡',
            '<strong>' + correct + ' dari ' + total + ' soal benar (' + pct + '%).</strong> ' + feedbackMsg) +
    ...
    document.getElementById('pbRetryBtn').addEventListener('click', function () {
        State.pb.idx = 0;
        State.pb.results = DATA.pembulatan.soal.map(function () { return { chosen: null, isCorrect: false }; });
        State.pb._opts = {};
        saveState();
        renderPembulatan(container);
    });
...
function renderEstimasi(container) {
    ...
    sec.innerHTML =
        ...
        buildProgressDots(scenarios.length, idx, scenarios.map(function (_, i) {
        ...
        '<div>' +
        '<p style="font-size:0.8rem;font-weight:700;letter-spacing:0.05em;text-transform:uppercase;color:var(--color-primary);margin-bottom:4px;">' + esc(scenario.label) + '</p>' +
        '<p style="font-size:0.95rem;margin-bottom:var(--space-2);">' + esc(scenario.context) + '</p>' +
        '</div>' +
        '<div style="text-align:right;flex-shrink:0;">' +
        '<span style="font-size:0.72rem;font-weight:700;text-transform:uppercase;color:var(--color-ink-muted);">Anggaranmu</span>' +
        '<div style="font-family:var(--font-mono);font-size:1.8rem;font-weight:700;color:var(--color-primary-strong);">' + formatRupiah(scenario.budget) + '</div>' +
        '</div>' +
        '</div>' +
        ...
        '</tr></thead><tbody>' +
        scenario.items.map(function (item) {
            return '<tr style="border-bottom:1px solid var(--color-border);">' +
                '<td style="padding:var(--space-2) var(--space-3);">' + esc(item.nama) + '</td>' +
                '<td style="text-align:right;padding:var(--space-2) var(--space-3);font-family:var(--font-mono);font-weight:600;color:var(--color-orange-strong);">' + formatRupiah(item.harga) + '</td>' +
                '</tr>';
        }).join('') +
        ...
        (sd.phase === 'revealed' ? formatRupiah(calcTotalBelanja(scenario.items)) : '— (belum diungkap)') +
        '</td>' +
        '</tr>' +
        '</tbody></table>' +
        '</div>';
...
function renderEstimasiSelectStrategy(sec, scenario, idx) {
    var stratHtml =
        ...
        DATA.estimasi.strategies.map(function (s) {
            return '<button type="button" class="strategy-card" data-level="' + s.value + '"' +
                ' aria-label="' + esc(s.label) + ': ' + esc(s.desc) + '">' +
                '<div class="strategy-card__label">' + esc(s.label) + '</div>' +
                '<div class="strategy-card__desc">' + esc(s.desc) + '</div>' +
                '<span class="strategy-card__example">' + esc(s.example) + '</span>' +
                '</button>';
        }).join('') +
        '</div>' +
        '</div>';
...
function renderEstimasiRounding(sec, scenario, idx, sd) {
    var strategy = DATA.estimasi.strategies.find(function (s) { return s.value === sd.strategy; });
    if (!strategy) return;
    ...
    var itemsHtml =
        ...
        '<span style="background:var(--color-primary-soft);color:var(--color-primary-strong);padding:3px 10px;border-radius:99px;font-size:0.8rem;font-weight:600;">' + esc(strategy.label) + '</span>' +
        '<button type="button" class="btn btn--ghost btn--small" id="changeStrategyBtn">Ganti Strategi</button>' +
        '</div>' +
        '<div class="item-rounding-list">' +
        scenario.items.map(function (item, i) {
            ...
            return '<div class="' + rowClass + '">' +
                ...
                '<span class="item-rounding-row__name">' + esc(item.nama) + '</span>' +
                '<span class="item-rounding-row__price">' + formatRupiah(item.harga) + '</span>' +
                '</div>' +
                '<div class="item-rounding-row__options" role="group" aria-label="Pilihan pembulatan ' + esc(item.nama) + '">' +
                optsCache[i].map(function (opt) {
                    var cls = 'option-btn';
                    if (isItemAnswered) {
                ...
                '</div>' +
                (isItemAnswered
                    ? '<div class="item-rounding-row__feedback is-visible ' + (chosen === correctRounded ? 'is-correct' : 'is-incorrect') + '">' +
                    (chosen === correctRounded
                        ? '✓ Tepat! ' + formatRupiah(item.harga) + ' dibulatkan ke ' + formatRupiah(correctRounded)
                        : '✗ Kurang tepat. ' + formatRupiah(item.harga) + ' seharusnya dibulatkan ke ' + formatRupiah(correctRounded)) +
                    '</div>'
                    : ''
                ) +
                '</div>';
    ...
    var decideHtml =
        '<div class="decision-panel" id="decisionPanel" ' + (allAnswered ? '' : 'style="opacity:0.4;pointer-events:none;"') + '>' +
        '<h4>Keputusan Anggaran</h4>' +
        '<p style="font-size:0.88rem;color:var(--color-ink-muted);margin-bottom:var(--space-3);">Berdasarkan estimasimu (' + (allAnswered ? formatRupiah(totalEstimasi) : '—') + '), apakah uang ' + formatRupiah(scenario.budget) + ' menurutmu cukup?</p>' +
        '<div class="decision-btns">' +
        '<button type="button" class="decision-btn decision-btn--cukup' + (sd.decision === 'cukup' ? ' is-selected' : '') + '" id="decisionCukupBtn" ' + (sd.phase === 'revealed' ? 'disabled' : '') + '>' +
        '✓ Cukup' +
        '</button>' +
        '<button type="button" class="decision-btn decision-btn--tidak' + (sd.decision === 'tidak' ? ' is-selected' : '') + '" id="decisionTidakBtn" ' + (sd.phase === 'revealed' ? 'disabled' : '') + '>' +
        '✗ Tidak Cukup' +
        '</button>' +
        '</div>' +
        ...
        (allAnswered && sd.decision ? '' : ' disabled') + '>Kunci Keputusan →</button>' +
        '</div>' +
        '</div>';
...
function renderEstimasiReveal(sec, scenario, idx, sd) {
    ...
    var dirLabel = dir === 'over' ? 'Overestimasi ↑' : dir === 'under' ? 'Underestimasi ↓' : 'Estimasi Tepat ✓';
    var dirBadgeClass = dir === 'over' ? 'deviation-badge--over' : dir === 'under' ? 'deviation-badge--under' : 'deviation-badge--exact';

    /* Feedback kontekstual */
    var feedbackMsg = buildEstimasiFeedback(scenario, sd, totalEstimasi, actualTotal, dir, devPct, decisionCorrect);
    ...
    var revealHtml =
        ...
        '<div class="comparison-card__value">' + formatRupiah(totalEstimasi) + '</div>' +
        ...
        '<div class="comparison-card__value">' + formatRupiah(actualTotal) + '</div>' +
        '<div style="margin-top:var(--space-2);font-size:0.78rem;">' +
        (actualOk
            ? '<span style="color:var(--color-success-strong);">≤ Anggaran</span>'
            : '<span style="color:var(--color-error-strong);">Melebihi Anggaran</span>') +
        ...
        '<div class="deviation-row__bar deviation-row__bar--' + (devPct <= 5 ? 'good' : devPct <= 15 ? 'ok' : 'bad') + '" style="width:' + Math.min(100, devPct * 2) + '%"></div>' +
        '</div>' +
        '<span class="deviation-row__value">' + formatRupiah(devAmt) + ' (' + devPct + '%)</span>' +
        '</div>' +

        '<div style="display:flex;align-items:center;gap:var(--space-3);flex-wrap:wrap;margin-bottom:var(--space-3);">' +
        '<span class="deviation-badge ' + dirBadgeClass + '">' + dirLabel + '</span>' +
        '<span class="decision-result-badge ' + (decisionCorrect ? 'decision-result-badge--correct' : 'decision-result-badge--incorrect') + '">' +
    ...
    /* Tombol lanjut */
    var nextLabel = idx < DATA.estimasi.scenarios.length - 1 ? 'Lanjut ke Skenario Berikutnya →' : 'Lihat Ringkasan Estimasi →';
    var btnRow = document.createElement('div');
    btnRow.className = 'btn-group btn-group--end';
    btnRow.innerHTML = '<button type="button" class="btn btn--primary" id="esNextBtn">' + nextLabel + '</button>';
...
function buildEstimasiFeedback(scenario, sd, totalEstimasi, actualTotal, dir, devPct, decisionCorrect) {
    var budget = scenario.budget;
    var strategyName = sd.strategy === 1000 ? 'ribuan' : sd.strategy === 5000 ? 'lima ribuan' : 'sepuluh ribuan';
    ...
    /* Arah deviasi */
    if (dir === 'over') {
        msg += 'Estimasimu lebih <strong>tinggi</strong> dari total sebenarnya (selisih ' + formatRupiah(deviationAmt(totalEstimasi, actualTotal)) + ' atau ' + devPct + '%). ' +
            'Ini disebut <em>overestimasi</em>. ';
    } else if (dir === 'under') {
        msg += 'Estimasimu lebih <strong>rendah</strong> dari total sebenarnya (selisih ' + formatRupiah(deviationAmt(totalEstimasi, actualTotal)) + ' atau ' + devPct + '%). ' +
    ...
    if (decisionCorrect) {
        ...
        if (sd.decision === 'tidak' && actualOk) {
            msg += '<strong>Keputusanmu kurang tepat:</strong> estimasimu yang terlalu tinggi membuatmu mengira uang tidak cukup, padahal total sebenarnya (' + formatRupiah(actualTotal) + ') masih di bawah anggaran (' + formatRupiah(budget) + '). ';
        } else {
            msg += '<strong>Keputusanmu kurang tepat:</strong> estimasimu yang terlalu rendah membuatmu mengira uang cukup, padahal total sebenarnya (' + formatRupiah(actualTotal) + ') melebihi anggaran (' + formatRupiah(budget) + '). ';
        }
    }

    /* Analisis strategi */
    if (devPct <= 3) {
        msg += 'Strategi pembulatan ke <strong>' + strategyName + '</strong> menghasilkan estimasi yang sangat akurat untuk situasi ini (selisih hanya ' + devPct + '%).';
    } else if (devPct <= 10) {
        msg += 'Strategi pembulatan ke <strong>' + strategyName + '</strong> memberikan estimasi yang cukup baik (selisih ' + devPct + '%).';
    } else if (devPct <= 20) {
        msg += 'Strategi pembulatan ke <strong>' + strategyName + '</strong> memberikan estimasi yang agak kasar (selisih ' + devPct + '%). Untuk anggaran ketat, pertimbangkan pembulatan ke ribuan agar lebih akurat.';
    } else {
        msg += 'Strategi pembulatan ke <strong>' + strategyName + '</strong> menghasilkan estimasi yang jauh meleset (selisih ' + devPct + '%). Pembulatan ke satuan yang terlalu besar dapat menyebabkan keputusan yang salah, seperti yang terjadi pada skenario ini.';
    }
...
function renderEstimasiSummary(container) {
    ...
    var rows = scenarios.map(function (sc, i) {
        ...
        return '<tr style="border-bottom:1px solid var(--color-border);">' +
            '<td style="padding:var(--space-2) var(--space-3);">' + esc(sc.label) + '</td>' +
            '<td style="padding:var(--space-2) var(--space-3);font-family:var(--font-mono);">' + strategyName + '</td>' +
            '<td style="padding:var(--space-2) var(--space-3);font-family:var(--font-mono);">' + formatRupiah(totalEst) + '</td>' +
            '<td style="padding:var(--space-2) var(--space-3);font-family:var(--font-mono);">' + formatRupiah(actualTotal) + '</td>' +
            '<td style="padding:var(--space-2) var(--space-3);text-align:center;">' +
            (decisionOk
    ...
    container.innerHTML =
        ...
        buildFeedbackBox(
            ...
            '<strong>' + correctCount + ' dari ' + scenarios.length + ' keputusan anggaran tepat.</strong> ' +
            (correctCount === scenarios.length
...
function renderTantangan(container) {
    ...
    var totalSelected = selectedIds.reduce(function (s, id) {
        var item = challenge.items.find(function (it) { return it.id === id; });
    ...
    container.innerHTML =
        ...
        buildProgressDots(challenges.length, idx, challenges.map(function (_, i) {
        ...
        '<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:var(--space-3);flex-wrap:wrap;">' +
        '<div>' +
        '<p style="font-size:0.8rem;font-weight:700;letter-spacing:0.05em;text-transform:uppercase;color:var(--color-primary);margin-bottom:4px;">' + esc(challenge.label) + '</p>' +
        '<p style="font-size:0.95rem;margin-bottom:0;">' + esc(challenge.context) + '</p>' +
        '</div>' +
        '<div style="text-align:right;flex-shrink:0;">' +
        '<span style="font-size:0.72rem;font-weight:700;text-transform:uppercase;color:var(--color-ink-muted);">Anggaranmu</span>' +
        '<div style="font-family:var(--font-mono);font-size:1.8rem;font-weight:700;color:var(--color-primary-strong);">' + formatRupiah(challenge.budget) + '</div>' +
        '</div>' +
        '</div>' +
        '</div>' +
        ...
        'Pilih minimal ' + challenge.minSelect + ' barang. Total sebenarnya akan ditampilkan setelah kamu mengunci pilihan.' +
        '</div>' +

        '<div class="budget-summary">' +
        '<div class="budget-summary__anggaran">' +
        '<span class="budget-summary__anggaran-label">Anggaran</span>' +
        '<span class="budget-summary__anggaran-value">' + formatRupiah(challenge.budget) + '</span>' +
        '</div>' +
        ...
        challenge.items.map(function (item) {
            ...
            var cardClass = 'challenge-item-card' + (isSelected ? ' is-selected' : '') + (td.locked ? ' is-locked' : '');
            if (td.locked) {
                cardClass += isSelected ? (withinBudget ? ' reveal-ok' : ' reveal-over-budget') : '';
            }
            return '<div class="' + cardClass + '" data-id="' + esc(item.id) + '" tabindex="0" ' +
                'role="checkbox" aria-checked="' + isSelected + '" aria-label="' + esc(item.nama) + ' ' + formatRupiah(item.harga) + '">' +
                '<span class="challenge-item-card__check" aria-hidden="true">' + (isSelected ? '✓' : '') + '</span>' +
                '<span class="challenge-item-card__name">' + esc(item.nama) + '</span>' +
                '<span class="challenge-item-card__price">' + formatRupiah(item.harga) + '</span>' +
                '</div>';
        }).join('') +
        '</div>' +
...
function buildTantanganReveal(challenge, td, totalSelected) {
    ...
    var unselected = challenge.items.filter(function (it) { return td.selected.indexOf(it.id) === -1; });
    var cheapestMissed = unselected.sort(function (a, b) { return a.harga - b.harga; })[0];
    ...
    var feedbackMsg = '';
    if (withinBudget) {
        feedbackMsg = '<strong>Pilihanmu sesuai anggaran!</strong> Total belanja (' + formatRupiah(totalSelected) + ') tidak melebihi anggaran ' + formatRupiah(challenge.budget) + ' (sisa ' + formatRupiah(challenge.budget - totalSelected) + '). ';
        if (couldAdd) {
            feedbackMsg += 'Dengan sisa anggaran tersebut, kamu masih bisa menambahkan <strong>' + esc(cheapestMissed.nama) + '</strong> (' + formatRupiah(cheapestMissed.harga) + ') jika diperlukan.';
        }
    } else {
        feedbackMsg = '<strong>Pilihanmu melebihi anggaran!</strong> Total belanja (' + formatRupiah(totalSelected) + ') melebihi anggaran ' + formatRupiah(challenge.budget) + ' sebesar ' + formatRupiah(over) + '. ';
        feedbackMsg += 'Cobalah hapus barang dengan harga tertinggi dari pilihanmu, atau ganti dengan barang yang lebih murah.';
    }

    var nextLabel = State.tn.idx < DATA.tantangan.challenges.length - 1 ? 'Tantangan Berikutnya →' : 'Lihat Ringkasan →';

    return '<div id="tantanganReveal">' +
        buildFeedbackBox(feedbackType, feedbackIcon, feedbackMsg) +
        '<div class="btn-group btn-group--end">' +
        '<button type="button" class="btn btn--primary" id="nextTantanganBtn">' + nextLabel + '</button>' +
        '</div>' +
        '</div>';
}
...
function renderTantanganSummary(container) {
    ...
    var rows = challenges.map(function (ch, i) {
        var td = State.tn.data[i];
        if (!td || !td.locked) return '';
        var total = td.selected.reduce(function (s, id) {
            var item = ch.items.find(function (it) { return it.id === id; });
            return s + (item ? item.harga : 0);
        }, 0);
        var ok = total <= ch.budget;
        if (ok) successCount++;
        return '<tr style="border-bottom:1px solid var(--color-border);">' +
            '<td style="padding:var(--space-2) var(--space-3);">' + esc(ch.label) + '</td>' +
            '<td style="padding:var(--space-2) var(--space-3);">' + formatRupiah(ch.budget) + '</td>' +
            '<td style="padding:var(--space-2) var(--space-3);">' + td.selected.length + ' barang</td>' +
            '<td style="padding:var(--space-2) var(--space-3);font-family:var(--font-mono);">' + formatRupiah(total) + '</td>' +
    ...
    container.innerHTML =
        ...
        buildFeedbackBox(
            ...
            '<strong>' + successCount + ' dari ' + challenges.length + ' tantangan berhasil (pilihan sesuai anggaran).</strong> ' +
...
function renderAsesmen(container) {
    ...
    container.innerHTML =
        ...
        '<p class="stage-head__goal">' + esc(DATA.asesmen.instruction) + '</p>' +
        '</div>' +

        buildProgressDots(soalList.length, idx, soalList.map(function (s) {
            var a = State.as.answers[s.id];
            return { chosen: a && a.locked ? 'done' : null, isCorrect: a && a.chosen === s.correct };
        })) +

        '<div class="panel">' +
        '<p style="font-size:0.8rem;font-weight:700;text-transform:uppercase;letter-spacing:0.05em;color:var(--color-primary);margin-bottom:var(--space-3);">Pertanyaan ' + (idx + 1) + ' dari ' + soalList.length + '</p>' +
        '<p style="font-size:1rem;font-weight:600;line-height:1.5;margin-bottom:var(--space-3);" id="asQuestion">' + esc(soal.pertanyaan) + '</p>' +
        '<div class="asesmen-options" role="radiogroup" aria-labelledby="asQuestion">' +
        soal.pilihan.map(function (p) {
            ...
            return '<button type="button" class="' + cls + '"' +
                ...
                ' data-opt="' + esc(p.id) + '"' +
                ' role="radio" aria-checked="' + (p.id === ans.chosen) + '">' +
                '<span class="asesmen-option__key" aria-hidden="true">' + p.id.toUpperCase() + '</span>' +
                '<span>' + esc(p.teks) + '</span>' +
                '</button>';
        }).join('') +
        '</div>' +
        ...
        (ans.locked
            ? buildFeedbackBox(
                ...
                (ans.chosen === soal.correct
                    ? '<strong>Tepat!</strong> '
                    : '<strong>Belum tepat.</strong> Jawaban yang benar adalah pilihan <strong>' + soal.correct.toUpperCase() + '</strong>. ') +
                esc(soal.explanation)
            )
            : ''
        ) +

        '<div class="btn-group btn-group--' + (idx > 0 ? 'spread' : 'end') + '" style="margin-top:var(--space-4);">' +
        (idx > 0 ? '<button type="button" class="btn btn--ghost" id="asPrevBtn">← Sebelumnya</button>' : '') +
        (!ans.locked
            ? '<button type="button" class="btn btn--primary" id="asLockBtn"' + (ans.chosen ? '' : ' disabled') + '>Kunci Jawaban</button>'
            : '<button type="button" class="btn btn--primary" id="asNextBtn">' + (idx < soalList.length - 1 ? 'Soal Berikutnya →' : 'Lihat Hasil →') + '</button>'
        ) +
        '</div>' +
        '</div>' +
        '</section>';
...
function renderAsesmenSummary(container) {
    ...
    var rows = soalList.map(function (soal, i) {
        ...
        return '<tr style="border-bottom:1px solid var(--color-border);">' +
            '<td style="padding:var(--space-2) var(--space-3);">Soal ' + (i + 1) + '</td>' +
            '<td style="padding:var(--space-2) var(--space-3);font-size:0.85rem;">' + esc(soal.pertanyaan.slice(0, 60)) + '…</td>' +
            '<td style="text-align:center;padding:var(--space-2) var(--space-3);">' +
            (ok ? '<span style="color:var(--color-success-strong);font-weight:700;">✓</span>' : '<span style="color:var(--color-error-strong);font-weight:700;">✗</span>') +
            '</td>' +
            '</tr>';
    ...
    container.innerHTML =
        ...
        buildFeedbackBox(feedbackType, feedbackType === 'success' ? '🎉' : '📊',
            '<strong>' + correct + ' dari ' + total + ' jawaban benar (' + pct + '%).</strong> ' + feedbackMsg
        ) +
...
function renderRefleksi(container) {
    ...
    container.innerHTML =
        ...
        '<p class="stage-head__goal">' + esc(DATA.refleksi.instruction) + '</p>' +
        '</div>' +

        '<div class="panel panel--compact panel--info">' +
        '<p style="margin:0;font-size:0.88rem;">' +
        'Jawab setidaknya <strong>' + minRequired + '</strong> dari ' + questions.length + ' pertanyaan berikut (' + answeredCount + '/' + questions.length + ' sudah diisi).' +
        '</p>' +
        '</div>' +
        ...
        questions.map(function (q) {
            ...
            return '<div class="refleksi-question' + (isDone ? ' is-answered' : '') + '">' +
                '<div class="refleksi-question__label">' + esc(q.label) + '</div>' +
                '<p class="refleksi-question__prompt">' + esc(q.prompt) + '</p>' +
                '<textarea class="input-textarea" data-qid="' + esc(q.id) + '" placeholder="' + esc(q.placeholder) + '" aria-label="' + esc(q.prompt) + '">' + esc(val) + '</textarea>' +
                '</div>';
        }).join('') +
        ...
        '</button>' +
        (canProceed ? '' : '<span style="font-size:0.82rem;color:var(--color-ink-muted);align-self:center;">Isi ' + (minRequired - answeredCount) + ' pertanyaan lagi untuk melanjutkan.</span>') +
        '</div>' +
        '</section>';
    ...
    document.getElementById('rfSaveBtn').addEventListener('click', function () {
        ...
        if (count >= minRequired) {
            ...
            showNotice('Isi minimal ' + minRequired + ' pertanyaan (masing-masing lebih dari 10 karakter) untuk melanjutkan.');
        }
    });
}
...
function renderSelesai(container) {
    var pbCorrect = State.pb.results.filter(function (r) { return r.isCorrect; }).length;
    ...
    container.innerHTML =
        ...
        '<p style="font-size:0.95rem;color:var(--color-ink-muted);">' + esc(DATA.meta.goal) + '</p>' +
        '</div>' +
        ...
        '<div class="score-row"><span class="score-row__label">Pembulatan (soal benar)</span><span class="score-row__value">' + pbCorrect + ' / ' + pbTotal + '</span></div>' +
        '<div class="score-row"><span class="score-row__label">Estimasi (keputusan tepat)</span><span class="score-row__value">' + esCorrect + ' / ' + esTotal + '</span></div>' +
        '<div class="score-row"><span class="score-row__label">Decision Challenge (dalam anggaran)</span><span class="score-row__value">' + tnCorrect + ' / ' + tnTotal + '</span></div>' +
        '<div class="score-row"><span class="score-row__label">Asesmen (jawaban benar)</span><span class="score-row__value">' + asCorrect + ' / ' + asTotal + '</span></div>' +
        '</div>' +
...
function buildStageNav() {
    ...
    list.innerHTML = STAGES.map(function (sid, i) {
        return '<li>' +
            '<button type="button" class="stage-nav__item" data-stage="' + sid + '">' +
