...
function formatDecShort(val) { return formatDec(val, 4); }
...
var STAGES = ['orientasi', 'eksplorasi', 'konversi', 'bandingkan', 'urutkan', 'tantangan', 'refleksi', 'selesai'];
var STAGE_LABELS = ['Orientasi', 'Eksplorasi', 'Konversi', 'Bandingkan', 'Urutkan', 'Tantangan', 'Refleksi', 'Selesai'];
...
function initExerciseArrays() {
  ...
  if (!State.bandingkanExercises || State.bandingkanExercises.length !== DATA.bandingkan.soal.length) {
...
function buildFracBlock(num, den, whole, size) {
  ...
  var ariaLabel = whole != null
    ? esc(whole) + ' dan ' + esc(num) + ' per ' + esc(den)
    : esc(num) + ' per ' + esc(den);
  var wholeHTML = whole != null ? '<span class="frac-block__whole">' + esc(String(whole)) + '</span>' : '';
  return '<span class="frac-block' + cls + '" role="img" aria-label="' + ariaLabel + '">' +
    wholeHTML +
    '<span class="frac-block__frac">' +
    '<span class="frac-block__num">' + esc(String(num)) + '</span>' +
    '<span class="frac-block__den">' + esc(String(den)) + '</span>' +
    '</span></span>';
}
...
function buildFracInline(num, den, whole) {
  ...
  return '<span class="frac-inline" role="img" aria-label="' + ariaLabel + '">' +
    wholeStr +
    '<span class="frac-num">' + esc(String(num)) + '</span>' +
    '<span class="frac-den">' + esc(String(den)) + '</span>' +
    '</span>';
}
...
function buildNumberDisplay(obj, size) {
  ...
  if (obj.type === 'decimal') {
    ...
    return '<span class="decimal-display' + cls + '" aria-label="' + formatDecShort(obj.value).replace(',', ' koma ') + '">' +
      esc(formatDecShort(obj.value)) + '</span>';
  }
  return '';
}
...
function buildAreaModel(num, den, size) {
  ...
  return '<div class="area-model' + cls + '" style="grid-template-columns: repeat(' + cols + ', 1fr);" role="img" aria-label="' + num + ' dari ' + den + ' bagian diarsir">' + cells + '</div>';
}
...
function buildDecimalBar(val) {
  ...
  return '<div class="decimal-bar" role="progressbar" aria-valuenow="' + pctStr + '" aria-valuemin="0" aria-valuemax="100" aria-label="' + pctStr + '%">' +
    '<div class="decimal-bar__fill" style="width:' + pctStr + '%"></div>' +
    '<span class="decimal-bar__label">' + pctStr + '%</span>' +
    '</div>';
}
...
function renderCurrentStage() {
  ...
  switch (State.currentStage) {
    ...
    case 'konversi': renderKonversi(container); break;
    case 'bandingkan': renderBandingkan(container); break;
    case 'urutkan': renderUrutkan(container); break;
    ...
    default: container.innerHTML = '<p style="padding:var(--space-5);color:var(--color-ink-muted);">Tahap tidak ditemukan.</p>';
  }
}
...
function renderOrientasi(container) {
  container.innerHTML = '<section aria-label="Orientasi">' +
...
function countExplored() {
  return Object.values(State.explorationData).filter(function (d) { return d.shown; }).length;
}
...
function renderEksplorasi(container) {
  ...
  var cards = fracs.map(function (f) {
    var isSelected = selected === f.id;
    var isExplored = State.explorationData[f.id] && State.explorationData[f.id].shown;
    var cls = 'frac-card' + (isSelected ? ' is-selected' : '') + (isExplored ? ' is-explored' : '');
    return '<button type="button" class="' + cls + '" data-frac="' + f.id + '" aria-pressed="' + isSelected + '">' +
      buildFracBlock(f.num, f.den, null, 'large') +
      '<span class="frac-card__label">' + f.num + '/' + f.den + '</span>' +
      '</button>';
  }).join('');
  ...
  var detailHTML = '';
  if (selected) {
    var frac = fracs.find(function (f) { return f.id === selected; });
    ...
    var visualHTML =
      ...
      '<div class="explore-visual__title">Model Area — ' + frac.num + '/' + frac.den + '</div>' +
      '<div class="explore-fraction-hero">' +
      buildFracBlock(frac.num, frac.den, null, 'hero') +
      '</div>' +
      '<div class="area-model-wrap">' + buildAreaModel(frac.num, frac.den) + '</div>' +
      '<p class="area-model__caption">' + frac.num + ' dari ' + frac.den + ' bagian diarsir</p>' +
      '</div>';

    var predictHTML = '';
    if (!data.shown) {
      var hint1HTML = data.attempts >= 1 ? '<div class="hint-box"><span class="hint-box__label">💡 Petunjuk:</span>' + esc(frac.hint) + '</div>' : '';
      predictHTML =
        ...
        '<label for="predictInput">Nilai desimal dari ' + frac.num + '/' + frac.den + ' =</label>' +
        ...
        (data.attempts >= 2 ?
          ...
          buildFeedbackBox('info', 'ℹ️', '<strong>Bantuan:</strong> ' + frac.num + ' ÷ ' + frac.den + ' = <strong>' + esc(decDisplay) + '</strong>. Coba ketikkan nilai ini.') +
          '</div>' : '') +
      ...
      predictHTML =
        ...
        '<span class="decimal-display--hero" style="color:var(--color-primary-strong);">' + esc(decDisplay) + '</span>' +
        '</div>' +
        '<div class="area-model-wrap">' + buildDecimalBar(fracVal) + '</div>' +
        buildFeedbackBox('info', '📘',
          '<strong>' + frac.num + ' ÷ ' + frac.den + ' = ' + esc(decDisplay) + '</strong><br>' +
          esc(frac.explanation) +
          (isRepeat ? '<br><em>Catatan: Ini adalah desimal berulang yang tidak pernah berhenti.</em>' : '')
        ) +
        '</div></div>';
    }
  ...
  container.innerHTML =
    ...
    '<div class="panel">' +
    '<h3>' + esc(DATA.eksplorasi.title) + '</h3>' +
    '<p style="font-size:0.9rem;color:var(--color-ink-muted);">' + esc(DATA.eksplorasi.instruction) + '</p>' +
    '<div class="explore-progress" style="margin-bottom:var(--space-3);">' +
    '<span>Dieksplorasi:</span>' +
    '<span class="explore-progress__count" id="exploredCount">' + explored + ' dari ' + fracs.length + '</span>' +
    '<span>pecahan</span>' +
    (canProceed ? ' <span style="color:var(--color-success-strong);font-weight:600;">✓ Siap lanjut!</span>' : ' <span style="font-size:0.8rem;">(minimal ' + minReq + ')</span>') +
    '</div>' +
    '<div class="frac-grid" id="fracGrid">' + cards + '</div>' +
    '</div>' +
    ...
    '<button type="button" class="btn btn--primary" id="nextExploreBtn" ' + (canProceed ? '' : 'disabled') + '>' +
  ...
  if (checkBtn) {
    ...
    checkBtn.addEventListener('click', function () {
      ...
      if (parsed.error === 'empty') {
        ...
        input.classList.add('has-error'); input.focus(); return;
      }
      ...
      if (correct) {
        ...
        if (dataEntry.attempts >= 2) {
          dataEntry.shown = true;
          saveState();
          feedbackEl.innerHTML = buildFeedbackBox('warning', '⚠️',
            'Jawaban kurang tepat. Nilai yang benar adalah <strong>' + esc(formatDecShort(fracVal)) + (isTerminating(frac.num, frac.den) ? '' : '...') + '</strong>. Perhatikan penjelasan di bawah ini.'
          );
          /* Tunda render untuk tampilkan feedback sebentar */
          setTimeout(function () { renderEksplorasi(container); }, 1200);
        } else {
          saveState();
          feedbackEl.innerHTML = buildFeedbackBox('error', '✗',
            'Belum tepat. Periksa kembali — perhatikan petunjuk yang muncul di bawah ini.'
          );
          /* Re-render untuk tampilkan petunjuk */
          setTimeout(function () { renderEksplorasi(container); }, 800);
        }
      }
    });
  }
...
function renderKonversi(container) {
  ...
  var allDone = State.konversiExercises.every(function (e) { return e.correct || e.revealed; });
  ...
  var s = soal[idx];
  var ex = State.konversiExercises[idx] || { attempts: 0, hintLevel: 0, correct: false, userInput: '', revealed: false };
  ...
  if (ex.hintLevel >= 1 && !done) {
    feedbackHTML += '<div class="hint-box"><span class="hint-box__label">💡 Petunjuk ' + ex.hintLevel + ':</span>' + esc(s.hints[ex.hintLevel - 1]) + '</div>';
  }
  if (done) {
    if (ex.correct) {
      feedbackHTML = buildFeedbackBox('success', '✓',
        '<strong>Tepat!</strong> ' + s.num + '/' + s.den + ' = ' + esc(s.display) + '<br>' + s.explanation
      );
    } else {
      feedbackHTML = buildFeedbackBox('warning', '📘',
        '<strong>Jawaban:</strong> ' + s.num + '/' + s.den + ' = <strong>' + esc(s.display) + '</strong><br>' + s.explanation +
        (s.terminating === false ? '<br><em>Catatan: 1/3 menghasilkan desimal berulang (0,333...). Ini bukan kesalahan — memang sifat pecahannya.</em>' : '')
      );
    }
  }
  ...
  container.innerHTML =
    ...
    '<div class="area-model-wrap">' + areaModel + '</div>' +
    '<div class="area-model-wrap" style="margin-top:var(--space-2);">' + buildDecimalBar(s.num / s.den) + '</div>' +
    '</div>' +
    '</div>' +
    ...
    'placeholder="' + esc(placeholderText) + '" ' +
    'value="' + esc(ex.userInput) + '" ' +
    (done ? 'disabled' : '') +
    ' aria-describedby="konversiErr" aria-label="Masukkan nilai desimal untuk ' + s.num + '/' + s.den + '">' +
    '</div>' +
    ...
    '<button type="button" class="btn btn--ghost" id="prevKonversiBtn" ' + (idx === 0 ? 'disabled' : '') + '>← Sebelumnya</button>' +
    (!done ?
      '<button type="button" class="btn btn--ghost btn--small" id="hintKonversiBtn" ' + (ex.hintLevel >= s.hints.length ? 'disabled' : '') + '>💡 Petunjuk</button>' :
      '') +
    '</div>' +
    '<div style="display:flex;gap:var(--space-2);flex-wrap:wrap;">' +
    (!done ?
  ...
  if (checkBtn) checkBtn.addEventListener('click', function () { checkKonversiAnswer(container, idx); });
...
function renderKonversiSummary(container) {
  ...
  var correctCount = exercises.filter(function (e) { return e.correct; }).length;
  var total = soal.length;
  var pct = Math.round((correctCount / total) * 100);
  var allCorrect = correctCount === total;

  var rows = soal.map(function (s, i) {
    var ex = exercises[i];
    var status = ex.correct ? '✓' : '✗';
    var statusCls = ex.correct ? 'color:var(--color-success-strong)' : 'color:var(--color-error-strong)';
    return '<tr>' +
      '<td>' + s.num + '/' + s.den + '</td>' +
      '<td>' + esc(s.display) + '</td>' +
      '<td style="' + statusCls + ';font-weight:600;">' + status + '</td>' +
      '</tr>';
  }).join('');
  ...
  container.innerHTML =
    ...
    buildFeedbackBox(
      ...
      allCorrect ? '🎉' : '📊',
      '<strong>' + correctCount + ' dari ' + total + ' soal dijawab benar (' + pct + '%)</strong>' +
      (allCorrect ? '<br>Semua konversi tepat! Kamu menguasai strategi konversi pecahan ke desimal.' :
...
function renderBandingkan(container) {
  ...
  var allDone = State.bandingkanExercises.every(function (e) { return e.checked; });
  ...
  var s = soal[idx];
  var ex = State.bandingkanExercises[idx] || { attempts: 0, correct: false, chosen: null, checked: false };
  ...
  var opButtons = ops.map(function (op) {
    ...
    return '<button type="button" class="' + cls + '" data-op="' + op + '" ' +
      'aria-label="' + ariaLabels[op] + '" ' +
      (isChecked ? 'disabled' : '') +
      '>' + opLabels[op] + '</button>';
  }).join('');
  ...
  if (isChecked) {
    ...
    if (ex.correct) {
      feedbackHTML = buildFeedbackBox('success', '✓',
        ...
        '<br><em style="font-size:0.85rem;">Strategi: ' + s.strategy + '</em>'
      );
    } else {
      feedbackHTML = buildFeedbackBox('error', '✗',
        '<strong>Kurang tepat.</strong> Jawaban yang benar: ' +
        '<strong>' + fmtNumberDisplay(s.a) + ' ' + esc(s.answer) + ' ' + fmtNumberDisplay(s.b) + '</strong><br>' +
        s.explanation +
        '<br><em style="font-size:0.85rem;">Strategi: ' + s.strategy + '</em>'
      );
    }
    /* Tampilkan perbandingan desimal */
    feedbackHTML += '<div class="comparison-result" style="margin-top:var(--space-3);">' +
      '<span style="font-family:var(--font-mono);">' + formatDecShort(valA) + '</span>' +
      '<span style="color:var(--color-primary-strong);">' + esc(s.answer) + '</span>' +
      '<span style="font-family:var(--font-mono);">' + formatDecShort(valB) + '</span>' +
      '</div>';
  } else if (ex.attempts >= 1 && !isChecked) {
    feedbackHTML = '<div class="hint-box"><span class="hint-box__label">💡 Petunjuk:</span>' + esc(s.hint) + '</div>';
  }
  ...
  container.innerHTML =
    ...
    '<div class="bandingkan-item__num">' + buildNumberDisplay(s.a, 'large') + '</div>' +
    '</div>' +
    '<span class="bandingkan-sep">?</span>' +
    '<div class="bandingkan-item">' +
    '<span class="bandingkan-item__label">Bilangan 2</span>' +
    '<div class="bandingkan-item__num">' + buildNumberDisplay(s.b, 'large') + '</div>' +
    '</div>' +
    '</div>' +

    '<div role="group" aria-label="Pilih operator perbandingan">' +
    '<div class="operator-choice">' + opButtons + '</div>' +
    '</div>' +

    feedbackHTML +

    '<div class="btn-group btn-group--spread" style="margin-top:var(--space-5);">' +
    '<button type="button" class="btn btn--ghost" id="prevBandingkanBtn" ' + (idx === 0 ? 'disabled' : '') + '>← Sebelumnya</button>' +
    '<div style="display:flex;gap:var(--space-2);flex-wrap:wrap;">' +
    (!isChecked ?
      '<button type="button" class="btn btn--primary" id="checkBandingkanBtn" ' + (!ex.chosen ? 'disabled' : '') + '>Periksa Jawaban</button>' :
      (idx < soal.length - 1 ?
  ...
  if (retryBtn) retryBtn.addEventListener('click', function () {
    State.bandingkanExercises[idx] = { attempts: 0, correct: false, chosen: null, checked: false };
    saveState();
    renderBandingkan(container);
  });

  var summaryBtn2 = document.getElementById('summaryBandingkanBtn');
  if (summaryBtn2) summaryBtn2.addEventListener('click', function () {
...
function renderBandingkanSummary(container) {
  ...
  container.innerHTML =
    ...
    '<div class="panel">' +
    buildFeedbackBox(
      correctCount === total ? 'success' : (correctCount >= Math.ceil(total * 0.6) ? 'info' : 'warning'),
      correctCount === total ? '🎉' : '📊',
      '<strong>' + correctCount + ' dari ' + total + ' perbandingan tepat (' + pct + '%)</strong>' +
      (correctCount < total ? '<br>Ingat: untuk membandingkan, ubah ke desimal dulu, lalu bandingkan digit per digit.' : '<br>Kamu berhasil membandingkan bilangan rasional dalam berbagai representasi!')
    ) +
...
function getProductById(id) {
  return DATA.urutkan.products.find(function (p) { return p.id === id; });
}
...
function buildSortCardHTML(pid, rank, feedbackSet, checkedDone) {
  ...
  if (p.sugar.type === 'fraction') {
    ...
    sugarDisplay = '<span style="font-weight:700;color:var(--color-primary-strong);">' + formatDecShort(p.sugar.value) + '</span>';
  }

  return '<div class="' + cardCls + '" draggable="' + (!checkedDone ? 'true' : 'false') + '" ' +
    'data-id="' + pid + '" id="sc-' + pid + '" ' +
    'aria-label="' + esc(p.name) + ', urutan ' + rank + '">' +
    '<span class="sort-card__handle" aria-hidden="true">⠿</span>' +
    '<span class="sort-card__rank">' + rank + '.</span>' +
    '<div class="sort-card__content">' +
    '<div class="sort-card__name">' + esc(p.name) + '</div>' +
    '<div class="sort-card__sugar">Gula: <strong>' + sugarDisplay + '</strong> sdm/sajian</div>' +
    '</div>' +
    '<div class="sort-card__move-btns" role="group" aria-label="Pindahkan ' + esc(p.name) + '">' +
    '<button type="button" class="sort-card__btn" data-move="up" data-id="' + pid + '" ' +
    'aria-label="Pindah ke atas" ' + (rank === 1 || checkedDone ? 'disabled' : '') + '>▲</button>' +
    '<button type="button" class="sort-card__btn" data-move="down" data-id="' + pid + '" ' +
    'aria-label="Pindah ke bawah" ' + (rank === State.urutkanOrder.length || checkedDone ? 'disabled' : '') + '>▼</button>' +
    '</div>' +
    statusIcon +
    '</div>';
}
...
function renderUrutkan(container) {
  ...
  var feedbackSet = (State.urutkanFeedback && State.urutkanFeedback.correctSet) ? State.urutkanFeedback.correctSet : null;
  ...
  var sortCardsHTML = State.urutkanOrder.map(function (pid, i) {
  ...
  if (checkedDone && feedbackSet) {
    if (correctCount === correctOrder.length) {
      feedbackHTML = buildFeedbackBox('success', '🎉',
        '<strong>Urutan benar!</strong> Kamu berhasil mengurutkan ' + correctOrder.length + ' produk dari kandungan gula terkecil ke terbesar.' +
        '<br>Kunci: konversikan semua ke desimal, lalu urutkan: <strong>0,25 &lt; 0,3 &lt; 0,35 &lt; 0,375 &lt; 0,45 &lt; 0,6</strong>'
      );
    } else {
      feedbackHTML = buildFeedbackBox('warning', '⚠️',
        '<strong>' + correctCount + ' dari ' + correctOrder.length + ' posisi sudah tepat.</strong><br>' +
        'Kartu yang disorot merah perlu dipindahkan. ' + esc(DATA.urutkan.checkHint) +
        ...
        DATA.urutkan.products.map(function (p) {
          var disp = p.sugar.type === 'fraction' ? p.sugar.num + '/' + p.sugar.den : formatDecShort(p.sugar.value);
          return '<div>' + esc(p.name) + ': ' + esc(disp) + ' = <strong>' + formatDecShort(p.decimal) + '</strong></div>';
        }).join('') +
        '</div></details>'
      );
    }
  ...
  container.innerHTML =
    ...
    '</div>' +
    '<div class="panel">' +
    '<h3>' + esc(DATA.urutkan.title) + '</h3>' +
    '<p style="font-size:0.88rem;color:var(--color-ink-muted);">' + esc(DATA.urutkan.context) + '</p>' +
    '<div class="feedback-box feedback-box--info" style="margin-bottom:var(--space-4);">' +
    '<span class="feedback-box__icon">ℹ️</span>' +
    '<div class="feedback-box__body"><strong>' + esc(DATA.urutkan.instruction) + '</strong><br>' +
    ...
    (State.urutkanAttempts > 0 && !checkedDone ?
      '<p style="font-size:0.85rem;color:var(--color-ink-muted);">Percobaan ke-' + State.urutkanAttempts + '</p>' : '') +

    '<div class="sort-list" id="sortList">' + sortCardsHTML + '</div>' +
  ...
  if (checkBtn) checkBtn.addEventListener('click', function () { checkUrutkanOrder(container); });
  ...
  if (retryBtn2) retryBtn2.addEventListener('click', function () {
    State.urutkanFeedback = null;
    State.urutkanDone = false;
    saveState();
    renderUrutkan(container);
...
function renderTantangan(container) {
  ...
  if (ex.checked) {
    if (ex.correct) {
      feedbackHTML = buildFeedbackBox('success', '✓', '<strong>Tepat!</strong><br>' + s.explanation);
    } else {
      feedbackHTML = buildFeedbackBox('error', '✗', '<strong>Belum tepat.</strong><br>' + s.explanation);
    }
  ...
  container.innerHTML =
    ...
    '<div class="challenge-story">' + s.story + '</div>' +

    '<p style="font-weight:600;margin-bottom:var(--space-2);">' + s.question + '</p>' +
    ...
    '<div class="btn-group btn-group--spread" style="margin-top:var(--space-5);">' +
    '<button type="button" class="btn btn--ghost" id="prevTantanganBtn" ' + (idx === 0 ? 'disabled' : '') + '>← Sebelumnya</button>' +
    '<div style="display:flex;gap:var(--space-2);flex-wrap:wrap;">' +
    (!ex.checked ?
      '<button type="button" class="btn btn--primary" id="checkTantanganBtn" ' +
      ((s.type === 'choice' && !ex.chosen) || (s.type === 'ordering' && !ex.order) ? 'disabled' : '') +
  ...
  container.querySelectorAll('[data-tantangan-move]').forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      var tmp = order[fromIdx2]; order[fromIdx2] = order[toIdx2]; order[toIdx2] = tmp;
      saveState();
      renderTantangan(container);
    });
  });
  ...
  if (checkBtn) checkBtn.addEventListener('click', function () { checkTantanganAnswer(container, idx); });
  ...
  if (retryBtn3) retryBtn3.addEventListener('click', function () {
    var s2 = soal[idx];
    ...
    State.tantanganExercises[idx] = {
      ...
      order: s2.type === 'ordering' ? (currentOrder ? currentOrder.slice() : s2.initialOrder.slice()) : null,
      checked: false
    };
    saveState();
    renderTantangan(container);
  ...
  if (summaryBtn3) summaryBtn3.addEventListener('click', function () {
...
function buildTantanganChoice(s, ex) {
  var isChecked = ex.checked;
  var optHTML = s.options.map(function (opt) {
    var cls = 'choice-btn';
    if (isChecked) {
    ...
    var iconContent = isChecked && opt.id === s.correct ? '✓' :
      (isChecked && opt.id === ex.chosen && opt.id !== s.correct ? '✗' : letter);
    return '<button type="button" class="' + cls + '" data-id="' + opt.id + '" ' +
      (isChecked ? 'disabled' : '') + '>' +
      '<span class="choice-btn__icon">' + iconContent + '</span>' +
      esc(opt.label) +
      '</button>';
  }).join('');
  return '<div class="challenge-options" role="group" aria-label="Pilihan jawaban">' + optHTML + '</div>';
}

function buildTantanganOrdering(s, ex) {
  var isChecked = ex.checked;
  var order = ex.order || s.items.map(function (it) { return it.id; });
  ...
  var rows = order.map(function (itemId, i) {
    var item = s.items.find(function (it) { return it.id === itemId; });
    if (!item) return '';
    var statusIcon = '';
    var rowCls = 'sort-card';
    if (isChecked) {
      if (correctSet2.has(itemId)) { rowCls += ' sort-card--correct'; statusIcon = '✓'; }
      else { rowCls += ' sort-card--incorrect'; statusIcon = '✗'; }
    }
    return '<div class="' + rowCls + '" style="cursor:default;">' +
      '<span class="sort-card__rank">' + (i + 1) + '.</span>' +
      '<div class="sort-card__content">' +
      '<div class="sort-card__name">' + esc(item.label) + '</div>' +
      '</div>' +
      '<div class="sort-card__move-btns">' +
      '<button type="button" class="sort-card__btn" data-tantangan-move="up" data-id="' + itemId + '" ' +
      (i === 0 || isChecked ? 'disabled' : '') + '>▲</button>' +
      '<button type="button" class="sort-card__btn" data-tantangan-move="down" data-id="' + itemId + '" ' +
      (i === order.length - 1 || isChecked ? 'disabled' : '') + '>▼</button>' +
      '</div>' +
      (statusIcon ? '<span style="font-weight:700;' + (statusIcon === '✓' ? 'color:var(--color-success-strong)' : 'color:var(--color-error-strong)') + ';">' + statusIcon + '</span>' : '') +
      '</div>';
  }).join('');
...
function checkTantanganAnswer(container, idx) {
  ...
  if (s.type === 'choice') {
    ...
    var correct = true;
    for (var i = 0; i < s.correctOrder.length; i++) {
      if (ex.order[i] !== s.correctOrder[i]) { correct = false; break; }
    }
    ex.correct = correct;
    ex.checked = true;
...
function renderTantanganSummary(container) {
  ...
  container.innerHTML =
    ...
    buildFeedbackBox(
      ...
      correctCount === total ? '🎉' : '📋',
      '<strong>' + correctCount + ' dari ' + total + ' tantangan diselesaikan dengan benar.</strong>' +
...
function renderRefleksi(container) {
  ...
  /* Summary stats */
  var konversiCorrect = State.konversiExercises.filter(function (e) { return e.correct; }).length;
  var bandingkanCorrect = State.bandingkanExercises.filter(function (e) { return e.correct; }).length;
  var tantanganCorrect = State.tantanganExercises.filter(function (e) { return e.correct; }).length;
  var urutkanDone = State.urutkanDone;

  var questionHTML = soal.map(function (q, i) {
    var saved = State.refleksiAnswers[q.id] || '';
    return '<div class="panel panel--compact refleksi-item">' +
      '<span class="refleksi-item__num">Pertanyaan ' + (i + 1) + ' dari ' + soal.length + '</span>' +
      '<label for="r-' + q.id + '" style="font-size:0.95rem;font-weight:600;display:block;margin-bottom:var(--space-3);">' +
      q.question + '</label>' +
      '<textarea id="r-' + q.id + '" class="input-textarea" placeholder="' + esc(q.placeholder) + '" ' +
      'data-rid="' + q.id + '">' + esc(saved) + '</textarea>' +
      '</div>';
  }).join('');
  ...
  container.innerHTML =
    ...
    '<div class="summary-card__val">' + konversiCorrect + '/' + DATA.konversi.soal.length + '</div>' +
    '<div class="summary-card__label">Konversi benar</div>' +
    '</div>' +
    '<div class="summary-card">' +
    '<div class="summary-card__val">' + bandingkanCorrect + '/' + DATA.bandingkan.soal.length + '</div>' +
    '<div class="summary-card__label">Perbandingan benar</div>' +
    '</div>' +
    '<div class="summary-card">' +
    '<div class="summary-card__val">' + tantanganCorrect + '/' + DATA.tantangan.soal.length + '</div>' +
    '<div class="summary-card__label">Tantangan benar</div>' +
    '</div>' +
    '<div class="summary-card">' +
    '<div class="summary-card__val" style="font-size:1.5rem;">' + (urutkanDone ? '✓' : '–') + '</div>' +
    ...
    esc(DATA.refleksi.note) + '</p>' +
    '</div>' +

    '<div class="refleksi-list">' + questionHTML + '</div>' +
...
function renderSelesai(container) {
  ...
  container.innerHTML =
    ...
    '<div class="summary-card"><div class="summary-card__val">' + explored + '/8</div><div class="summary-card__label">Pecahan dieksplorasi</div></div>' +
    '<div class="summary-card"><div class="summary-card__val">' + konversiCorrect + '/8</div><div class="summary-card__label">Konversi benar</div></div>' +
    '<div class="summary-card"><div class="summary-card__val">' + bandingkanCorrect + '/6</div><div class="summary-card__label">Perbandingan benar</div></div>' +
    '<div class="summary-card"><div class="summary-card__val">' + tantanganCorrect + '/4</div><div class="summary-card__label">Tantangan benar</div></div>' +
    '</div>' +
