...
function formatNumber(n) {
  ...
  while (str.length > 3) { parts.unshift(str.slice(str.length - 3)); str = str.slice(0, str.length - 3); }
...
var STAGES = ['orientasi', 'eksplorasi', 'rumusUn', 'latihanUn', 'ekspSn', 'latihanSn', 'tantangan', 'refleksi', 'selesai'];
var STAGE_LABELS = ['Orientasi', 'Pola', 'Rumus Uₙ', 'Latihan Uₙ', 'Eksplorasi Sₙ', 'Latihan Sₙ', 'Tantangan', 'Refleksi', 'Selesai'];
...
function saveState() {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(State)); } catch (e) { /* Abaikan. */ }
}
...
function initExerciseArrays() {
  ...
  if (!State.eksplorasiRevealCount || State.eksplorasiRevealCount.length !== nCtx) {
    State.eksplorasiRevealCount = DATA.eksplorasi.konteks.map(function () { return 0; });
  }
  if (!State.eksplorasiDone || State.eksplorasiDone.length !== nCtx) {
    State.eksplorasiDone = DATA.eksplorasi.konteks.map(function () { return false; });
  }
  if (!State.rumusUnStepInputs || State.rumusUnStepInputs.length !== DATA.rumusUn.steps.length) {
    State.rumusUnStepInputs = DATA.rumusUn.steps.map(function () { return ''; });
  }
  if (!State.rumusUnStepDone || State.rumusUnStepDone.length !== DATA.rumusUn.steps.length) {
    State.rumusUnStepDone = DATA.rumusUn.steps.map(function () { return false; });
  }
}
...
function buildSeqTermsHTML(ctx, revealCount) {
  ...
  for (var i = 0; i < ctx.terms.length; i++) {
    ...
    html += '<div class="seq-term">' +
      '<div class="' + valCls + '">' + (revealed ? formatNumber(ctx.terms[i]) : '?') + '</div>' +
  ...
  if (isDone) {
    return '<div class="panel panel--compact" style="margin-top:var(--space-4);">' +
      buildFeedbackBox('success', '&#10003;',
        ...
        'Suku pertama <strong>a = ' + formatNumber(ctx.a) + '</strong> ' + esc(ctx.unit) +
        ', rasio <strong>r = ' + ctx.r + '</strong>.<br>' +
        '<em>Setiap suku dikalikan secara konstan sebesar ' + ctx.r + '.</em>'
      ) + '</div>';
  }
  if (revealCount >= 2) {
    return '<div class="panel panel--compact" style="margin-top:var(--space-4);">' +
...
function renderEksplorasi(container) {
  ...
  container.innerHTML =
    ...
    '<div class="panel">' +
    '<p style="font-size:0.88rem;color:var(--color-ink-muted);margin-bottom:var(--space-3);">' + esc(DATA.eksplorasi.instruction) + '</p>' +
    '<div style="display:flex;gap:var(--space-2);flex-wrap:wrap;margin-bottom:var(--space-4);">' + tabsHTML + '</div>' +
    ...
    '<span style="font-size:1.8rem;">' + ctx.icon + '</span>' +
    '<div>' +
    '<span class="badge-chip">' + esc(ctx.badge) + '</span>' +
    '<p style="margin:var(--space-1) 0 0;font-size:0.92rem;">' + ctx.story + '</p>' +
    '</div></div>' +

    '<div class="seq-display">' + termsHTML + '</div>' +
  ...
  if (hintBtn) hintBtn.addEventListener('click', function () {
    ...
    if (fb) fb.innerHTML = buildFeedbackBox('warning', '💡',
      '<strong>Petunjuk a:</strong> ' + ctx.hint_a + '<br>' +
      '<strong>Petunjuk r:</strong> ' + ctx.hint_r
    );
  });

  var checkBtn = document.getElementById('checkArBtn');
  if (checkBtn) checkBtn.addEventListener('click', function () { checkArAnswer(container, idx); });
...
function renderRumusUn(container) {
  ...
  var tableRows = data.tableRows.map(function (row) {
    var nCell = '<td class="cell-highlight">' + esc(String(row.n)) + '</td>';
    var bentukCell = '<td>' + row.bentuk + '</td>';
    var nilai_rCell = '<td style="color:var(--color-blue-strong);font-weight:600;">' + row.nilai_r + '</td>';
    var unCell = row.Un !== null
  ...
  if (!State.rumusUnFormulaShown) {
    ...
    stepHTML =
      ...
      (!ex ?
        ...
        '<input type="text" inputmode="numeric" id="stepInput" class="input-text" value="' + esc(stepInputValue) + '" placeholder="..." style="max-width:160px;text-align:center;font-family:var(--font-mono);font-size:1.1rem;">' +
  ...
  container.innerHTML =
    ...
    'Barisan: <strong>' + esc(data.konteks) + '</strong>' +
    '</p>' +
  ...
  if (checkBtn) checkBtn.addEventListener('click', function () { checkRumusUnStep(container, step); });
...
function checkRumusUnStep(container, step) {
  ...
  if (correct) {
    ...
    fb.innerHTML = buildFeedbackBox('error', '✗', 'Belum tepat. Coba perhatikan polanya pada tabel di atas.');
  }
}
...
function renderLatihanUn(container) {
  ...
  if (ex.hintLevel >= 1 && !done) {
    feedbackHTML += '<div class="hint-box"><span class="hint-box__label">💡 Petunjuk ' + ex.hintLevel + ':</span>' + s.hints[ex.hintLevel - 1] + '</div>';
  }
  if (done) {
  ...
  container.innerHTML =
    ...
    (s.konteks ? '<div class="challenge-story" style="margin-bottom:var(--space-3);">📌 ' + s.konteks + '</div>' : '') +

    '<div class="un-exercise">' +
    '<div class="un-exercise__seq">' + esc(s.barisan_display) + '</div>' +
    '<div class="un-exercise__question">' + s.question + '</div>' +
    '</div>' +
    ...
    'value="' + esc(ex.userInput) + '" placeholder="..." ' +
    (done ? 'disabled' : '') + ' aria-label="Masukkan jawaban">' +
    '</div>' +
    ...
    '<button type="button" class="btn btn--ghost" id="prevUnBtn" ' + (idx === 0 ? 'disabled' : '') + '>← Sebelumnya</button>' +
    (!done ? '<button type="button" class="btn btn--ghost btn--small" id="hintUnBtn" ' + (ex.hintLevel >= s.hints.length ? 'disabled' : '') + '>💡 Petunjuk</button>' : '') +
    '</div>' +
    '<div style="display:flex;gap:var(--space-2);flex-wrap:wrap;">' +
    (!done
  ...
  if (!done) { var inp = document.getElementById('latihanUnInput'); if (inp) setTimeout(function () { inp.focus(); }, 50); }

  var input = document.getElementById('latihanUnInput');
  if (input) input.addEventListener('keydown', function (e) {
    if (e.key === 'Enter') { e.preventDefault(); var ck = document.getElementById('checkUnBtn'); if (ck) ck.click(); }
  });

  attachInputExerciseEvents(container, 'latihanUnInput', 'latihanUnErr', 'checkUnBtn', 'hintUnBtn', 'prevUnBtn', 'nextUnBtn', 'summaryUnBtn',
    State.latihanUnExercises, idx, soal, 'latihanUn', container);
}
...
function renderLatihanUnSummary(container) {
  ...
  container.innerHTML =
    ...
    '<div class="summary-card"><div class="summary-card__val">' + correctCount + '</div><div class="summary-card__label">Soal benar dari ' + total + '</div></div>' +
    '<div class="summary-card"><div class="summary-card__val">' + Math.round((correctCount / total) * 100) + '%</div><div class="summary-card__label">Skor</div></div>' +
    '</div>' +
    (correctCount === total
      ? buildFeedbackBox('success', '🎉', '<strong>Luar biasa!</strong> Kamu berhasil menguasai perhitungan Uₙ pada barisan geometri.')
      : buildFeedbackBox('info', '📘', 'Bagus! Teruslah berlatih. Kamu bisa kembali mengerjakan soal yang belum benar.')) +
    '</div>' +
  ...
  document.getElementById('retryUnBtn').addEventListener('click', function () {
    State.latihanUnIdx = 0; saveState(); renderLatihanUn(container);
  });
  document.getElementById('nextToEkspSnBtn').addEventListener('click', function () {
    completeStage('latihanUn'); navigateTo('ekspSn');
  });
}
...
function renderEkspSn(container) {
  ...
  var rsnTerms = terms.map(function (t) { return t * r; });
  rsnTerms.push(terms[n - 1] * r); rsnTerms.shift(); // shift by one position
  /* Actually rSₙ = ar + ar² + ... + arⁿ, so each term is terms[i] * r */
  var rsnDisplayTerms = terms.map(function (t) { return t * r; });
  ...
  var resultHTML = '<div class="geom-row">' +
    ...
    '<div class="geom-cell geom-cell--rsn">' + rsnDisplayTerms[n - 1] + '</div>' +
    '<span class="geom-plus">−</span>' +
    '<div class="geom-cell geom-cell--sn">' + terms[0] + '</div>' +
    '<span class="geom-plus">=</span>' +
    '<div class="geom-cell geom-cell--result">' + resultVal + '</div>' +
    '</div>';

  var derivVisualHTML = '<div class="geom-deriv-wrap">';
  if (step >= 0) {
    derivVisualHTML += '<p style="font-size:0.8rem;font-family:var(--font-mono);color:var(--color-ink-muted);margin-bottom:var(--space-1);">— Deret Geometri (S' + n + ') —</p>';
    derivVisualHTML += snHTML;
  }
  if (step >= 1) {
    derivVisualHTML += '<p style="font-size:0.8rem;font-family:var(--font-mono);color:var(--color-ink-muted);margin:var(--space-2) 0 var(--space-1);">— Dikalikan r = ' + r + ' (r×S' + n + ') —</p>';
    derivVisualHTML += rsnHTML;
  }
  if (step >= 2) {
    derivVisualHTML += '<hr class="geom-divider-line">';
    derivVisualHTML += '<p style="font-size:0.8rem;font-family:var(--font-mono);color:var(--color-ink-muted);margin-bottom:var(--space-1);">— Pengurangan: suku yang sama saling menghapus —</p>';
  ...
  var stepsHTML = data.steps.slice(0, step + 1).map(function (st, i) {
    return '<div class="panel panel--compact" style="' + (i === step ? 'border-color:var(--color-primary);' : '') + '">' +
      '<strong>' + esc(st.title) + '</strong>' +
      '<p style="font-family:var(--font-mono);font-size:0.92rem;margin:var(--space-2) 0 0;white-space:pre-line;">' + esc(st.desc) + '</p>' +
      '</div>';
  }).join('');
  ...
  if (State.ekspSnFormulaShown) {
    ...
    verifikHTML =
      ...
      (!State.ekspSnVerifDone ?
        ...
        '<input type="text" inputmode="numeric" id="verifInput" class="input-text" value="' + esc(State.ekspSnVerifInput) + '" placeholder="..." style="max-width:120px;text-align:center;font-family:var(--font-mono);font-size:1.1rem;">' +
  ...
  container.innerHTML =
    ...
    '<div class="panel">' +
    '<h3>' + esc(data.title) + '</h3>' +
    '<p style="font-size:0.88rem;color:var(--color-ink-muted);">' + esc(data.instruction) + '</p>' +
    ...
    'Barisan: ' + data.barisan.join(', ') + ' (a = ' + data.a + ', r = ' + data.r + ', n = ' + data.n_demo + ')' +
    '</p>' +
  ...
  if (nextDeriv) nextDeriv.addEventListener('click', function () {
    State.ekspSnStep = Math.min(data.steps.length - 1, State.ekspSnStep + 1);
    saveState(); renderEkspSn(container);
  });

  var showFormula = document.getElementById('showFormulaBtn');
  if (showFormula) showFormula.addEventListener('click', function () {
    State.ekspSnFormulaShown = true; saveState(); renderEkspSn(container);
  });
  ...
  if (checkVerif) checkVerif.addEventListener('click', function () {
    ...
    var parsed = parseInputInt(inp.value);
    if (parsed.error) { fb.innerHTML = buildFeedbackBox('error', '✗', 'Masukkan bilangan bulat.'); return; }
    if (parsed.value === data.verifikasi.answer) {
      State.ekspSnVerifDone = true; saveState(); renderEkspSn(container);
    } else {
      fb.innerHTML = buildFeedbackBox('error', '✗', 'Belum tepat. Gunakan petunjuk untuk mencoba lagi.');
    }
  });

  var hintVerif = document.getElementById('hintVerifBtn');
  if (hintVerif) hintVerif.addEventListener('click', function () {
    var fb = document.getElementById('verifFeedback');
  ...
  if (verifInp) {
    verifInp.addEventListener('keydown', function (e) {
      if (e.key === 'Enter') { e.preventDefault(); if (checkVerif) checkVerif.click(); }
    });
    setTimeout(function () { verifInp.focus(); }, 50);
  }

  var nextSn = document.getElementById('nextToLatihanSn');
  if (nextSn) nextSn.addEventListener('click', function () {
    completeStage('ekspSn'); navigateTo('latihanSn');
  });
}
...
function buildDerivRow(label, termValues, cellClass) {
  ...
  for (var i = 0; i < termValues.length; i++) {
    ...
    html += '<div class="geom-cell geom-cell--' + cellClass + '">' + formatNumber(termValues[i]) + '</div>';
  }
  html += '</div>';
  return html;
}
...
function buildDerivRowCancel(label, termValues, cellClass, isCancelledFn) {
  ...
  for (var i = 0; i < termValues.length; i++) {
    if (i > 0) html += '<span class="geom-plus">+</span>';
    var cls = isCancelledFn(i) ? 'geom-cell geom-cell--cancel' : ('geom-cell geom-cell--' + cellClass);
...
function renderLatihanSn(container) {
  ...
  container.innerHTML =
    ...
    '<div class="btn-group btn-group--spread" style="margin-top:var(--space-5);">' +
    '<div style="display:flex;gap:var(--space-2);flex-wrap:wrap;">' +
    '<button type="button" class="btn btn--ghost" id="prevSnBtn" ' + (idx === 0 ? 'disabled' : '') + '>← Sebelumnya</button>' +
    (!done ? '<button type="button" class="btn btn--ghost btn--small" id="hintSnBtn" ' + (ex.hintLevel >= s.hints.length ? 'disabled' : '') + '>💡 Petunjuk</button>' : '') +
    '</div>' +
    '<div style="display:flex;gap:var(--space-2);flex-wrap:wrap;">' +
    (!done
  ...
  if (!done) { var inp = document.getElementById('latihanSnInput'); if (inp) setTimeout(function () { inp.focus(); }, 50); }

  var input = document.getElementById('latihanSnInput');
  if (input) input.addEventListener('keydown', function (e) {
    if (e.key === 'Enter') { e.preventDefault(); var ck = document.getElementById('checkSnBtn'); if (ck) ck.click(); }
  });

  attachInputExerciseEvents(container, 'latihanSnInput', 'latihanSnErr', 'checkSnBtn', 'hintSnBtn', 'prevSnBtn', 'nextSnBtn', 'summarySnBtn',
    State.latihanSnExercises, idx, soal, 'latihanSn', container);
}
...
function renderLatihanSnSummary(container) {
  ...
  container.innerHTML =
    ...
    '</div>' +
    (correctCount === total
      ? buildFeedbackBox('success', '🎉', '<strong>Luar biasa!</strong> Kamu menguasai perhitungan Sₙ pada deret geometri!')
      : buildFeedbackBox('info', '📘', 'Teruslah berlatih! Kamu bisa kembali mengerjakan soal yang tersisa.')) +
    '</div>' +
  ...
  document.getElementById('retrySnBtn').addEventListener('click', function () {
    State.latihanSnIdx = 0; saveState(); renderLatihanSn(container);
  });
  document.getElementById('nextToTantanganBtn').addEventListener('click', function () {
    completeStage('latihanSn'); navigateTo('tantangan');
  });
}
...
function attachInputExerciseEvents(container, inputId, errId, checkId, hintId, prevId, nextId, summaryId, exercises, idx, soal, stateKey, renderTarget) {
  var checkBtn = document.getElementById(checkId);
  if (checkBtn) checkBtn.addEventListener('click', function () { checkInputExercise(renderTarget, idx, exercises, soal, stateKey); });
...
function checkInputExercise(container, idx, exercises, soal, stateKey) {
  ...
  errEl.textContent = '';
  if (!input.value.trim()) { errEl.textContent = 'Ketikkan jawabanmu terlebih dahulu.'; input.focus(); return; }

  var parsed = parseInputInt(input.value);
  if (parsed.error) { errEl.textContent = 'Masukkan bilangan bulat (tanpa titik/koma).'; input.focus(); return; }
  ...
  if (parsed.value === s.answer) {
    ...
    else if (ex.hintLevel < s.hints.length) ex.hintLevel = Math.min(ex.hintLevel + 1, s.hints.length);
  }
  saveState();
...
function renderTantangan(container) {
  ...
  if (s.type === 'input') {
    questionHTML =
      '<p style="font-size:0.95rem;font-weight:600;margin-bottom:var(--space-3);">' + s.question + '</p>' +
      '<div class="un-answer-row">' +
      '<input type="text" inputmode="numeric" id="tantanganInput" class="input-text" ' +
      'value="' + esc(ex.userInput) + '" placeholder="..." ' +
      (done ? 'disabled' : '') + ' style="max-width:180px;font-family:var(--font-mono);font-size:1.1rem;">' +
      '<span style="font-size:0.88rem;color:var(--color-ink-muted);">' + esc(s.unit) + '</span>' +
      '</div>' +
    ...
    var optionsHTML = s.options.map(function (opt) {
      ...
      return '<button type="button" class="' + cls + '" data-opt="' + opt.id + '" ' + (done ? 'disabled' : '') + '>' +
        '<span class="choice-btn__icon">' + opt.id.replace('opt_', '').toUpperCase() + '</span>' +
  ...
  if (!done && ex.attempts >= 1 && s.type === 'input') {
    feedbackHTML = '<div class="hint-box"><span class="hint-box__label">💡 Petunjuk:</span>' + s.hint + '</div>';
  }
  if (done) {
    feedbackHTML = ex.correct
      ? buildFeedbackBox('success', '✓', '<strong>Tepat!</strong> ' + s.explanation)
  ...
  container.innerHTML =
    ...
    '<div class="btn-group btn-group--spread" style="margin-top:var(--space-5);">' +
    '<button type="button" class="btn btn--ghost" id="prevTBtn" ' + (idx === 0 ? 'disabled' : '') + '>← Sebelumnya</button>' +
    '<div style="display:flex;gap:var(--space-2);">' +
    (!done ? '<button type="button" class="btn btn--ghost btn--small" id="hintTBtn">💡 Petunjuk</button>' : '') +
    (!done
  ...
  container.querySelectorAll('[data-opt]').forEach(function (btn) {
    btn.addEventListener('click', function () { ex.chosen = btn.dataset.opt; saveState(); renderTantangan(container); });
  });
  ...
  if (checkBtn) checkBtn.addEventListener('click', function () { checkTantangan(container, idx); });

  var prevBtn = document.getElementById('prevTBtn');
  if (prevBtn) prevBtn.addEventListener('click', function () { State.tantanganIdx = Math.max(0, idx - 1); saveState(); renderTantangan(container); });

  var nextBtn = document.getElementById('nextTBtn');
  if (nextBtn) nextBtn.addEventListener('click', function () { State.tantanganIdx = Math.min(soal.length - 1, idx + 1); saveState(); renderTantangan(container); });

  var summaryBtn = document.getElementById('summaryTBtn');
  if (summaryBtn) summaryBtn.addEventListener('click', function () { State.tantanganIdx = soal.length; saveState(); renderTantangan(container); });
...
function checkTantangan(container, idx) {
  ...
  if (s.type === 'input') {
    var input = document.getElementById('tantanganInput');
    if (!input || !input.value.trim()) { if (errEl) errEl.textContent = 'Ketikkan jawabanmu.'; return; }
    var parsed = parseInputInt(input.value);
    if (parsed.error) { if (errEl) errEl.textContent = 'Masukkan bilangan bulat.'; return; }
    ex.userInput = input.value;
    ex.attempts += 1;
    ex.correct = parsed.value === s.answer;
    ex.checked = true;
  } else if (s.type === 'choice') {
    if (!ex.chosen) { if (errEl) errEl.textContent = 'Pilih salah satu jawaban terlebih dahulu.'; return; }
...
function renderRefleksi(container) {
  ...
  var itemsHTML = data.soal.map(function (q, i) {
    var savedVal = State.refleksiAnswers[q.id] || '';
    return '<div class="panel panel--compact refleksi-item">' +
      '<span class="refleksi-item__num">Pertanyaan ' + (i + 1) + '</span>' +
      '<p style="font-size:0.95rem;margin-bottom:var(--space-3);">' + q.question + '</p>' +
      '<textarea class="input-textarea" id="refleksi_' + q.id + '" placeholder="' + esc(q.placeholder) + '" rows="3">' + esc(savedVal) + '</textarea>' +
      '</div>';
  }).join('');
  ...
  container.innerHTML =
    ...
    '<p style="margin:0;font-size:0.9rem;">' + data.note + '</p>' +
    '</div>' +

    '<div class="refleksi-list">' + itemsHTML + '</div>' +

    (State.refleksiSaved ?
      '<div style="margin:var(--space-4) 0;">' + buildFeedbackBox('success', '✓', 'Refleksimu telah disimpan.') + '</div>' : '') +

    '<div class="btn-group btn-group--spread">' +
    '<button type="button" class="btn btn--outline-primary" id="saveRefleksiBtn">💾 Simpan Refleksi</button>' +
    '<button type="button" class="btn btn--primary btn--large" id="nextToSelesaiBtn" ' + (!State.refleksiSaved ? 'disabled' : '') + '>Selesai →</button>' +
    '</div></section>';

  data.soal.forEach(function (q) {
    var ta = document.getElementById('refleksi_' + q.id);
    if (ta) ta.addEventListener('input', function () { State.refleksiAnswers[q.id] = ta.value; });
  });
  ...
  if (nextBtn) nextBtn.addEventListener('click', function () {
    completeStage('refleksi'); navigateTo('selesai');
  });
}
...
function renderSelesai(container) {
  ...
  container.innerHTML =
    ...
    '<div class="summary-card"><div class="summary-card__val">' + correctUn + '/' + DATA.latihanUn.soal.length + '</div><div class="summary-card__label">Latihan Uₙ</div></div>' +
    '<div class="summary-card"><div class="summary-card__val">' + correctSn + '/' + DATA.latihanSn.soal.length + '</div><div class="summary-card__label">Latihan Sₙ</div></div>' +
    '<div class="summary-card"><div class="summary-card__val">' + correctT + '/' + DATA.tantangan.soal.length + '</div><div class="summary-card__label">Tantangan</div></div>' +
    '<div class="summary-card"><div class="summary-card__val">' + totalScore + '/' + maxScore + '</div><div class="summary-card__label">Total Soal Benar</div></div>' +
    '</div>' +
  ...
  if (reviewBtn) reviewBtn.addEventListener('click', function () { navigateTo('orientasi'); });
}
...
function showNotice(msg) {
  ...
  noticeTimer = setTimeout(function () { el.classList.remove('is-visible'); }, 2800);
}
