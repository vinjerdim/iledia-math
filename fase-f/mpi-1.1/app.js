...
function initExerciseArrays() {
  ...
  if (!State.eksplorasiRevealCount || State.eksplorasiRevealCount.length !== DATA.eksplorasi.konteks.length) {
  ...
  if (!State.eksplorasiInputs || State.eksplorasiInputs.length !== DATA.eksplorasi.konteks.length) {
    State.eksplorasiInputs = DATA.eksplorasi.konteks.map(function () { return null; });
  }
...
function renderEksplorasi(container) {
  ...
  if (checkBtn) checkBtn.addEventListener('click', function () { checkAbAnswer(container, idx); });
...
function renderRumusUn(container) {
  ...
  /* Tabel pola */
  var tableRows = data.tableRows.map(function (row, i) {
  ...
  container.innerHTML =
    ...
    'Barisan: <strong>' + esc(data.konteks) + '</strong> (a = ' + data.a + ', b = ' + data.b + ')' +
    '</p>' +
...
function renderLatihanUn(container) {
  ...
  var s  = soal[idx];
  var ex = State.latihanUnExercises[idx] || { attempts: 0, hintLevel: 0, correct: false, userInput: '', revealed: false };
...
function renderLatihanUnSummary(container) {
  ...
  container.innerHTML =
    ...
    '</div>' +
    (allCorrect
      ? buildFeedbackBox('success', '🎉', '<strong>Luar biasa!</strong> Kamu berhasil menguasai perhitungan Uₙ.')
...
function renderEkspSn(container) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    forwardHTML += (i > 0 ? '<span class="gauss-plus">+</span>' : '') +
      '<div class="' + fwCls + '">' + terms[i] + '</div>';
    backHTML += (i > 0 ? '<span class="gauss-plus">+</span>' : '') +
      '<div class="' + bkCls + '">' + terms[n - 1 - i] + '</div>';
    var sumVal = terms[i] + terms[n - 1 - i];
    sumHTML += (i > 0 ? '<span class="gauss-plus">+</span>' : '') +
      '<div class="gauss-cell gauss-cell--sum">' + sumVal + '</div>';
  }
  forwardHTML += '</div>';
  backHTML    += '</div>';
  sumHTML     += '<span class="gauss-plus">=</span><div class="gauss-cell gauss-cell--sum" style="min-width:52px;">' + (n * (terms[0] + terms[n-1])) + '</div></div>';

  /* Step reveal */
  var stepsHTML = steps.slice(0, step + 1).map(function (st, i) {
    return '<div class="panel panel--compact" style="' + (i === step ? 'border-color:var(--color-primary);' : '') + '">' +
      '<strong>' + esc(st.title) + '</strong>' +
      '<p style="font-family:var(--font-mono);font-size:0.95rem;margin:var(--space-2) 0 0;">' + esc(st.desc) + '</p>' +
      '</div>';
  }).join('');
  ...
  container.innerHTML =
    ...
    'Barisan: ' + data.barisan.join(', ') + ' (a = ' + data.a + ', b = ' + data.b + ', n = ' + data.n_demo + ')' +
    '</p>' +

    '<div class="gauss-wrap">' +
    (step >= 0 ? '<p style="font-size:0.82rem;font-family:var(--font-mono);color:var(--color-ink-muted);text-align:center;">— Deret Maju (S₈) —</p>' + forwardHTML : '') +
    (step >= 1 ? '<p style="font-size:0.82rem;font-family:var(--font-mono);color:var(--color-ink-muted);text-align:center;margin-top:var(--space-2);">— Deret Mundur (S₈) —</p>' + backHTML : '') +
    (step >= 2 ? '<p style="font-size:0.82rem;font-family:var(--font-mono);color:var(--color-ink-muted);text-align:center;margin-top:var(--space-2);">— 2S₈ (penjumlahan kolom) —</p>' + sumHTML : '') +
    '</div>' +
  ...
  if (nextGauss) nextGauss.addEventListener('click', function () {
  ...
  if (verifInp) verifInp.addEventListener('keydown', function (e) {
...
function renderLatihanSnSummary(container) {
  ...
  container.innerHTML =
    ...
    '</div>' +
    (allCorrect
      ? buildFeedbackBox('success', '🎉', '<strong>Luar biasa!</strong> Kamu menguasai perhitungan Sₙ!')
...
function renderRefleksi(container) {
  ...
  if (saveBtn) saveBtn.addEventListener('click', function () {
    ...
    var hasAny = Object.values(State.refleksiAnswers).some(function (v) { return v && v.trim().length > 0; });
